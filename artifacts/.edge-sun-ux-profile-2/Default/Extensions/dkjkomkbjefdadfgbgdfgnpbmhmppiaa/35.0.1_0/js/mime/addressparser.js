...
function addressparser(str) {
    ...
    tokens.forEach(function(token, index) {
        if (token.type === 'operator' && token.value === ',') {
            // Check if the comma is part of the name by looking at surrounding tokens
            const prevToken = tokens[index - 1];
            const nextToken = tokens[index + 1];

            if (
                prevToken && prevToken.type === 'text' &&
                nextToken && nextToken.type === 'text'
            ) {
                // Comma is part of the name, treat it as text
                address.push(token);
            } else {
                // Comma is a delimiter, split the address
                if (address.length) {
