...
export type OfflineScanSyncStatus =
  | "SYNCED_VALID"
  | "SYNCED_INVALID"
  | "REPLAY_SUSPECT"
  | "SYNC_PROCESSING"
...
const OFFLINE_SYNC_STATUS_VALUES = new Set<OfflineScanSyncStatus>([
  "SYNCED_VALID",
  "SYNCED_INVALID",
  "REPLAY_SUSPECT",
  "SYNC_PROCESSING",
...
function normalizedOfflineCapturedUrl(value: string) {
  ...
  if (
    !/^[A-Za-z0-9][A-Za-z0-9._:-]{2,119}$/.test(bid)
...
function normalizeOfflineScanRequest(params: OfflineScanSyncRequest) {
  ...
  const events = params.events.map((event) => {
    ...
    let approximateLocation: OfflineSyncApproximateLocation | undefined;
  ...
  const body = { schemaVersion: 1 as const, bundleId, deviceId, events };
...
function transformOfflineSyncResponse(data: unknown, traceId: string): OfflineScanSyncResponse {
  ...
  const countNames = ["received", "duplicates", "verified", "valid", "invalid", "pending", "rejected"] as const;
  const counts = Object.fromEntries(countNames.map((name) => [name, Number(record?.[name])])) as Record<typeof countNames[number], number>;
  if (
    !record
    || record.ok !== true
  ...
  const results = rawResults.map((value): OfflineScanSyncResult => {
    ...
    if (
      (terminal && (!verifiedAt || !clientEventId || !bid))
    ...
    const sealStatus = ["CLOSED", "OPENED", "UNKNOWN"].includes(String(item.seal_status))
      ? String(item.seal_status) as OfflineScanSyncResult["sealStatus"]
      : "UNKNOWN";
    return {
      ok: item.ok,
      clientEventId,
...
export class NexIdClient {
  ...
  syncOfflineScans(
    ...
    return this.request<OfflineScanSyncResponse>("/api/v1/sdk/offline-sync", {
      method: "POST",
      body,
      context: options,
      idempotencyKey,
