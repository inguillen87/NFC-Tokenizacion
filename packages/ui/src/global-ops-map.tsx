  ...
  return (
    <Card className="overflow-hidden p-4 md:p-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-sm font-semibold text-white">Global Ops Map {webglReady ? "WebGL" : "Fallback"}</p>
          ...
          <div className="relative h-[29rem]">
              ...
              {visibleRoutes.map((route) => {
                ...
                return (
                  <path key={route.id} d={curve(a, b, route.risk > 0 ? 56 : 42)} stroke={riskStroke} strokeWidth={route.risk > 0 ? 2.8 : 2} fill="none" strokeDasharray="4 5" opacity="0.95">
                    {playback ? <animate attributeName="stroke-dashoffset" values="0;-28" dur="2.2s" repeatCount="indefinite" /> : null}
                  </path>
                );
              })}
              {visiblePoints.map((point) => {
                ...
                return (
                  <g key={point.id}>
                    ...
                    <circle cx={dot.x} cy={dot.y} r={isSelected ? 18 : 12} fill="none" stroke="rgba(255,255,255,.22)">
                      <animate attributeName="opacity" values="0.8;0.2;0.8" dur={isSelected ? "1.8s" : "3.5s"} repeatCount="indefinite" />
                    </circle>
                  </g>
                );
              })}
            ...
            {!webglReady ? (
              <div className="absolute left-3 top-3 rounded-lg border border-amber-300/25 bg-amber-500/10 px-3 py-1 text-[11px] text-amber-100">
                WebGL unavailable in browser/runtime. Showing operational fallback view.
              </div>
            ) : null}
            <div className="absolute inset-x-0 bottom-0 border-t border-white/10 bg-slate-950/75 px-3 py-2 text-[11px] text-slate-300">
              Scatter/arc/trips simulation with clustered hubs and capped routes for performance ({visibleRoutes.length} routes rendered).
            </div>
          </div>
        </div>
