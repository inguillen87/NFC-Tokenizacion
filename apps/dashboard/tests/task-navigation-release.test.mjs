...
test("navigation labels and notes exist in all supported locales", () => {
  for (const locale of ["es-AR", "en", "pt-BR"]) {
    ...
    assert.equal(releaseCopy(locale).cards.length, 10);
...
test("public version marker agrees with the displayed release", async () => {
  ...
  assert.equal(marker.requiredApiRelease, "2026.09.23-api-supplier-requests.2");
  ...
  assert.equal(marker.scope, "supplier-request-clarifications-and-actionable-inbox");
