...
test("navigation labels and notes exist in all supported locales", () => {
  for (const locale of ["es-AR", "en", "pt-BR"]) {
    ...
    assert.equal(releaseCopy(locale).cards.length, 9);
...
test("public version marker agrees with the displayed release", async () => {
  ...
  assert.equal(marker.requiredApiRelease, "2026.09.22-api-support-workflow.2");
  ...
  assert.equal(marker.scope, "direct-ticket-entry-with-pending-command-preservation");
