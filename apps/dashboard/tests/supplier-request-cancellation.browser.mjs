...
const output=resolve(process.env.QA_OUTPUT||'artifacts/supplier-request-cancellation-browser');await mkdir(output,{recursive:true});
...
const report={localOnly:true,syntheticData:true,actualComponents:['SupplierRequestWorkspace','SupplierRequestCancellation'],actualClientContract:true,httpBoundaryMocked:true,navigationStubbed:true,realNextServer:false,realDatabase:false,productionTested:false,checks:[],views:[],clientErrors:[],blockedRequests:[]};
...
async function beginCancel(page){await page.getByTestId('supplier-cancel-start').click();await page.getByTestId('supplier-cancel-reason').waitFor();}
async function reviewCancel(page,reason='El proyecto requiere otra construcción. <b>Texto literal</b>'){
 await page.getByTestId('supplier-cancel-reason').fill(reason);await page.getByTestId('supplier-cancel-review').click();await page.getByTestId('supplier-cancel-confirmation').waitFor();
}
try{
 for(const theme of ['light','dark'])for(const width of [320,390,1440]){
  const row=submitted(),test=await scenario(width,theme,openedOptions(row));await reviewReady(test.page);
  check(test.state.cancelReads.length===0&&test.state.cancelWrites.length===0,'Cancellation does not poll or prefetch on opening a request');
  await beginCancel(test.page);check(await test.page.getByRole('button',{name:'Actualizar bandeja',exact:true}).isDisabled(),'Open cancellation locks parent refresh and sibling edits');
  check(await test.page.getByTestId('supplier-request-review-refresh').isDisabled(),'Open cancellation suspends clarification controls');
  await reviewCancel(test.page);check(test.state.cancelWrites.length===0,'Reviewing reason and scope performs no write');
  check(await test.page.getByTestId('supplier-cancel-confirmation').locator('h3').evaluate(n=>document.activeElement===n),'Confirmation moves keyboard focus to heading');
  await inspect(test.page,'cancel-confirmation',width,theme);
  await test.page.getByRole('button',{name:'Volver al motivo',exact:true}).click();check((await test.page.getByTestId('supplier-cancel-reason').inputValue()).includes('Texto literal'),'Returning preserves reason');
  test.page.once('dialog',d=>d.dismiss());await test.page.getByTestId('supplier-cancel-close').click();check(await test.page.getByTestId('supplier-cancel-panel').count()===1,'Dismiss discard dialog keeps wizard');
  test.page.once('dialog',d=>d.accept());await test.page.getByTestId('supplier-cancel-close').click();await tick(test.page);
  check(await test.page.getByTestId('supplier-cancel-start').evaluate(n=>document.activeElement===n),'Explicit close restores keyboard focus to opener');
  check(test.state.cancelWrites.length===0,'Leaving without final confirmation has no cancellation mutation');
  await beginCancel(test.page);await reviewCancel(test.page);test.state.cancelMode='delayed';
  await test.page.getByTestId('supplier-cancel-confirm').evaluate(button=>{button.click();button.click();});await waitFor(()=>test.state.cancelPending.length===1);
  check(test.state.cancelWrites.length===1,'Synchronous double confirmation creates one operation');await test.state.releaseCancellation();
  await test.page.getByTestId('supplier-cancel-receipt').waitFor();await reviewReady(test.page);
  check(test.state.cancelCommits===1&&row.status==='cancelled','Validated receipt updates the same commercial request');
  check(!await test.page.getByTestId('supplier-cancel-start').count()&&!await test.page.getByTestId('supplier-request-prepare').count()&&!await test.page.getByTestId('supplier-request-review-message').count(),'Cancelled request offers no cancel, prepare or clarification mutation');
  check((await test.page.getByTestId('supplier-request-review-state').textContent()).includes('cancelada'),'History explicitly labels cancellation');
  check(test.state.writes.length===0&&test.state.orderWrites.length===0&&test.state.reviewWrites.length===0,'Cancellation never submits a technical order, changes fields, or sends clarification');
  await test.page.getByTestId('supplier-request-inbox-filter').selectOption('cancelled');check(await test.page.getByTestId('supplier-request-open').count()===1,'Cancelled filter includes the confirmed record');
  await inspect(test.page,'cancelled-receipt',width,theme);await test.context.close();
 }
 for(const readMode of ['unavailable','foreign']){
  const row=submitted(),test=await scenario(390,'light',openedOptions(row));await reviewReady(test.page);test.state.cancelReadMode=readMode;await test.page.getByTestId('supplier-cancel-start').click();
  await test.page.getByTestId('supplier-cancel-panel').getByRole('alert').waitFor();check(test.state.cancelWrites.length===0&&!await test.page.getByTestId('supplier-cancel-confirm').count(),'Unavailable/untrusted snapshot cannot authorize cancellation: '+readMode);
  await test.page.getByTestId('supplier-cancel-close').click();check(await test.page.getByTestId('supplier-cancel-start').count()===1,'Unavailable rollout leaves a usable back action');await test.context.close();
 }
 const conflicted=submitted(),cas=await scenario(390,'light',openedOptions(conflicted));await reviewReady(cas.page);await beginCancel(cas.page);await reviewCancel(cas.page,'Motivo preservado ante cambios');
 const event=reviewEvent(conflicted,1);cas.state.reviewEvents.set(conflicted.id,[event]);conflicted.review_summary=reviewSummary([event]);
 await cas.page.getByTestId('supplier-cancel-confirm').click();await cas.page.getByTestId('supplier-cancel-panel').getByRole('alert').waitFor();
 check(await cas.page.getByTestId('supplier-cancel-reason').inputValue()==='Motivo preservado ante cambios'&&cas.state.cancelCommits===0,'Concurrent clarification rejects stale cancellation without discarding reason');
 await cas.page.getByTestId('supplier-cancel-refresh').click();await cas.page.getByTestId('supplier-cancel-review').waitFor();await cas.page.getByTestId('supplier-cancel-review').click();await cas.page.getByTestId('supplier-cancel-confirm').click();await cas.page.getByTestId('supplier-cancel-receipt').waitFor();await reviewReady(cas.page);
 check(cas.state.cancelWrites[1].body.expected_review_revision===1&&cas.state.cancelWrites[0].key!==cas.state.cancelWrites[1].key,'After a certain conflict, explicit reconfirmation uses fresh review revision and new operation');
 check((await cas.page.getByTestId('supplier-request-review-history').textContent()).includes(event.message),'Cancellation keeps the previous clarification history readable');await cas.context.close();
 for(const mode of ['commit-lost','malformed']){
  const row=submitted(),test=await scenario(390,'dark',openedOptions(row));await reviewReady(test.page);await beginCancel(test.page);await reviewCancel(test.page);test.state.cancelMode=mode;await test.page.getByTestId('supplier-cancel-confirm').click();await test.page.getByTestId('supplier-cancel-retry').waitFor();
  check(test.state.cancelCommits===1&&!await test.page.getByTestId('supplier-cancel-close').count(),'Lost or malformed receipt stays uncertain and prevents an ordinary close');
  test.state.cancelMode='forbidden';await test.page.getByTestId('supplier-cancel-retry').click();await test.page.getByTestId('supplier-cancel-retry').waitFor();check(await test.page.getByRole('button',{name:'Actualizar bandeja',exact:true}).isDisabled(),'Later denial cannot clear prior uncertainty');
  test.state.cancelMode='success';await test.page.getByTestId('supplier-cancel-retry').click();await test.page.getByTestId('supplier-cancel-receipt').waitFor();
  check(test.state.cancelCommits===1&&new Set(test.state.cancelWrites.map(x=>x.key)).size===1&&new Set(test.state.cancelWrites.map(x=>JSON.stringify(x.body))).size===1,'Every uncertain retry uses identical key/body and obtains only original cancellation receipt');await test.context.close();
 }
 const lostRow=submitted(),lost=await scenario(390,'light',openedOptions(lostRow));await reviewReady(lost.page);await beginCancel(lost.page);await reviewCancel(lost.page);lost.state.cancelMode='commit-lost';await lost.page.getByTestId('supplier-cancel-confirm').click();await lost.page.getByTestId('supplier-cancel-exit-uncertain').waitFor();
 lost.page.once('dialog',d=>d.accept());await lost.page.getByTestId('supplier-cancel-exit-uncertain').click();await tick(lost.page);check(lost.state.cancelWrites.length===1&&lost.state.cancelCommits===1,'Explicit uncertain exit neither retries nor reverses the committed operation');
 await lost.page.getByTestId('supplier-request-open').click();await lost.page.getByTestId('supplier-cancel-receipt').waitFor();check(lost.state.cancelWrites.length===1,'Readback after uncertain exit discovers cancelled status without another write');await lost.context.close();
 for(const status of ['draft','provisioned']){
  const row=status==='draft'?item():submitted({status:'provisioned',order_id:randomUUID()}),test=await scenario(390,'light',openedOptions(row));check(await test.page.getByTestId('supplier-cancel-start').count()===0,'No cancellation offered for '+status);check(test.state.cancelReads.length===0&&test.state.cancelWrites.length===0,'No cancellation reads/writes for '+status);await test.context.close();
 }
 const a=submitted(),b=submitted({title:'Otro expediente'}),late=await scenario(390,'light',openedOptions(a,{rows:[a,b]}));await reviewReady(late.page);late.state.cancelReadMode='delayed';await late.page.getByTestId('supplier-cancel-start').click();await waitFor(()=>late.state.cancelPending.length===1);
 late.state.cancelReadMode='success';await late.page.evaluate(id=>window.__qaContext({initialRequestId:id}),b.id);await late.page.getByTestId('supplier-request-detail-heading').filter({hasText:b.title}).waitFor();await reviewReady(late.page);await late.page.evaluate(id=>window.__qaContext({initialRequestId:id}),a.id);await late.page.getByTestId('supplier-request-detail-heading').filter({hasText:a.title}).waitFor();await reviewReady(late.page);await late.state.releaseCancellation();await tick(late.page);
 check(await late.page.getByTestId('supplier-cancel-panel').count()===0&&late.state.cancelWrites.length===0,'Late cancelled-context availability response cannot reopen a wizard after A to B to A');await late.context.close();
 const questionRow=submitted(),ev=reviewEvent(questionRow,1),dirty=await scenario(390,'light',openedOptions(questionRow,{history:[[questionRow.id,[ev]]]}));await reviewReady(dirty.page);await dirty.page.getByTestId('supplier-request-review-message').fill('Respuesta local todavía sin guardar');await dirty.page.getByTestId('supplier-cancel-start').click();
 check(dirty.state.cancelReads.length===0&&dirty.state.cancelWrites.length===0&&await dirty.page.getByTestId('supplier-request-review-message').inputValue()==='Respuesta local todavía sin guardar','Cancellation cannot silently discard a dirty company response');await dirty.context.close();
