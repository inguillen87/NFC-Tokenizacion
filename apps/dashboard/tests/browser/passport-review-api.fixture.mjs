...
const apiRoot=process.env.NEXID_QA_API_ROOT;if(!apiRoot||process.argv[2]!=='--local-qa')throw Error('Explicit local QA source required');
const {installEphemeralE2eSqlExecutor}=await import(pathToFileURL(join(apiRoot,'apps/api/src/lib/db.ts')).href);
const {readStudio,studioReadView,mutateStudio,studioFailure}=await import(pathToFileURL(join(apiRoot,'apps/api/src/lib/passport-editorial-service.ts')).href);
...
const dir=await mkdtemp(join(tmpdir(),'nexid-passport-browser-')),password=randomBytes(24).toString('hex'),port=15987;
...
await pool.query((await readFile(join(apiRoot,'apps/api/db/migrations/20260918120000_0104_passport_editorial.sql'),'utf8')).replaceAll('\r\n','\n'));
await pool.query((await readFile(join(apiRoot,'apps/api/db/migrations/20260918123000_0105_passport_editorial_guards.sql'),'utf8')).replaceAll('\r\n','\n'));
...
const server=http.createServer(async(req,res)=>{
 const url=new URL(req.url,'http://localhost:4487'),path=url.pathname;
 ...
 const token=req.headers.authorization||'',who=token==='Bearer local-studio-editor'?'editor':token==='Bearer local-studio-reviewer'?'reviewer':token==='Bearer local-studio-viewer'?'viewer':token==='Bearer local-studio-publisher'?'publisher':null;
 if(!who)return reply(401,{ok:false});
 const actor={actorId:'qa_'+who,label:who==='editor'?'Editor de producto QA':who==='reviewer'?'Revisor independiente QA':'Consulta QA',canEdit:who==='editor'||who==='publisher',canReview:who==='reviewer',canPublish:who==='publisher'};
 if(path==='/auth/session')return reply(200,{ok:true,session:{id:actor.actorId,email:who+'@example.invalid',label:actor.label,role:'tenant-admin',tenantId:tenant,tenantSlug:'qa-company',permissions:who==='viewer'?['batches:read']:who==='reviewer'?['batches:read','batch.product.review']:who==='publisher'?['batches:read','batch.product.configure','batch.product.publish']:['batches:read','batch.product.configure'],deniedPermissions:[],mfaVerified:true,setupCompleted:true,isDemo:false}});
...
server.listen(4487,'127.0.0.1',()=>console.log('LOCAL_STUDIO_FULLSTACK_READY_4487_POSTGRES_15987'));
