...
test("tenant account menu renders as a top-level drawer above CRM layers", () => {
  ...
  assert.match(menuSource, /const ACCOUNT_MENU_VERSION = "drawer-v9-fixed-portal"/);
...
test("global CSS prevents dashboard maps from covering account drawer", () => {
  ...
  assert.match(globalsSource, /\.nexid-account-layer\s*\{[\s\S]*z-index:\s*2147483646 !important/);
  ...
  assert.match(globalsSource, /\.nexid-account-backdrop\s*\{[\s\S]*z-index:\s*2147483645 !important/);
