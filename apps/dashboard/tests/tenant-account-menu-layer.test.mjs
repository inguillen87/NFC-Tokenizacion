...
test("tenant account menu renders as a top-level drawer above CRM layers", () => {
  ...
  assert.match(menuSource, /const ACCOUNT_MENU_VERSION = "drawer-v13-native-top-layer"/);
  ...
  assert.match(menuSource, /node\.style\.setProperty\("filter", "saturate\(0\.72\) brightness\(0\.44\) blur\(0\.5px\)", "important"\)/);
  assert.match(menuSource, /node\.style\.setProperty\("opacity", "0\.28", "important"\)/);
...
test("global CSS prevents dashboard maps from covering account drawer", () => {
  ...
  assert.match(globalsSource, /\.nexid-account-dialog::backdrop\s*\{[\s\S]*rgba\(2, 6, 23, 0\.88\) !important/);
  ...
  assert.match(globalsSource, /body\.nexid-account-menu-open \.nexid-crm-shell,\s*\.nexid-crm-shell\[data-account-menu-suppressed="true"\]\s*\{[^}]*filter:\s*saturate\(0\.72\) brightness\(0\.44\) blur\(0\.5px\) !important/);
  assert.match(globalsSource, /body\.nexid-account-menu-open \.nexid-crm-shell,\s*\.nexid-crm-shell\[data-account-menu-suppressed="true"\]\s*\{[^}]*opacity:\s*0\.28 !important/);
