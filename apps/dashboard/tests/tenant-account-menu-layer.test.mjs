...
test("tenant account menu renders as a top-level drawer above CRM layers", () => {
  ...
  assert.match(menuSource, /root\.setAttribute\("data-account-menu-modal-ready", "true"\)/);
  assert.match(menuSource, /root\.removeAttribute\("data-account-menu-modal-ready"\)/);
  ...
  assert.match(menuSource, /forceAccountMenuModalLayer\(promotedRoot, dialogRef\.current, layerRef\.current, panelRef\.current\)/);
  ...
  assert.match(menuSource, /const ACCOUNT_MENU_VERSION = "drawer-v16-immediate-modal-layer"/);
  ...
  assert.doesNotMatch(menuSource, /body\.nexid-account-menu-open > :not\(#nexid-account-menu-root\):not\(script\):not\(style\),[\s\S]*visibility:\s*hidden !important/);
  assert.match(menuSource, /user-select:\s*none !important/);
  assert.match(menuSource, /const ACCOUNT_DIALOG_STYLE: CSSProperties = \{/);
  assert.match(menuSource, /useRef<HTMLDialogElement \| null>/);
  assert.match(menuSource, /data-account-menu-dialog="native"/);
  assert.match(menuSource, /data-testid="tenant-account-menu-dialog"/);
  assert.match(menuSource, /className="nexid-account-dialog"/);
  assert.match(menuSource, /showModal\(\)/);
  assert.match(menuSource, /dialog\.close\(\)/);
  assert.match(menuSource, /function activateAccountMenuDialog\(dialog: HTMLDialogElement \| null\)/);
  assert.match(menuSource, /data-account-menu-modal-state", "native-top-layer"/);
  assert.match(menuSource, /data-account-menu-modal-state", "fixed-fallback"/);
  assert.match(menuSource, /function forceAccountMenuModalLayer/);
  assert.match(menuSource, /forceAccountMenuModalLayer\(root,\s*dialogRef\.current \|\| root\.querySelector<HTMLDialogElement>/);
  assert.match(menuSource, /window\.setTimeout\(\(\) => \{/);
  ...
  assert.match(menuSource, /role="document"/);
  ...
  assert.match(menuSource, /width:\s*"min\(100vw, 38rem\)"/);
  assert.match(menuSource, /height:\s*"100dvh"/);
  assert.match(menuSource, /maxWidth:\s*"min\(100vw, 38rem\)"/);
  ...
  assert.match(menuSource, /openRef\.current = true/);
  assert.match(menuSource, /openRef\.current = false/);
  assert.match(menuSource, /onPointerDownCapture=\{\(\) => \{/);
  assert.match(menuSource, /prepareMenuPortalRoot\(\)/);
  assert.match(menuSource, /onMouseDown=\{\(event\) => \{/);
  assert.match(menuSource, /onPointerUp=\{\(event\) => \{/);
  assert.match(menuSource, /activateMenu\(event\)/);
  ...
  assert.doesNotMatch(menuSource, /node\.style\.setProperty\("display", "none", "important"\)/);
  assert.match(menuSource, /node\.style\.setProperty\("z-index", "0", "important"\)/);
  assert.match(menuSource, /node\.style\.setProperty\("filter", "saturate\(0\.6\) brightness\(0\.28\) blur\(1px\)", "important"\)/);
  assert.match(menuSource, /node\.style\.setProperty\("opacity", "0\.12", "important"\)/);
  assert.doesNotMatch(menuSource, /node\.style\.setProperty\("visibility", "hidden", "important"\)/);
  assert.doesNotMatch(menuSource, /node\.style\.removeProperty\("display"\)/);
  ...
  assert.match(menuSource, /\.nexid-account-dialog::backdrop/);
...
test("global CSS prevents dashboard maps from covering account drawer", () => {
  ...
  assert.doesNotMatch(globalsSource, /body\.nexid-account-menu-open > :not\(#nexid-account-menu-root\):not\(script\):not\(style\),[\s\S]*visibility:\s*hidden !important/);
  assert.match(globalsSource, /\.nexid-account-dialog\s*\{[\s\S]*position:\s*fixed !important/);
  assert.match(globalsSource, /\.nexid-account-dialog\s*\{[\s\S]*width:\s*100vw !important/);
  assert.match(globalsSource, /\.nexid-account-dialog\s*\{[\s\S]*height:\s*100dvh !important/);
  assert.match(globalsSource, /\.nexid-account-dialog::backdrop\s*\{[\s\S]*rgba\(2, 6, 23, 0\.98\) !important/);
  assert.match(globalsSource, /\.nexid-account-dialog\[data-account-menu-modal-state\]\s*\{[\s\S]*z-index:\s*2147483647 !important/);
  ...
  assert.match(globalsSource, /#nexid-account-menu-root\[data-account-menu-modal-ready="true"\]/);
  ...
  assert.match(globalsSource, /\.nexid-account-layer\s*\{[\s\S]*width:\s*min\(100vw, 38rem\) !important/);
  assert.match(globalsSource, /\.nexid-account-layer\s*\{[\s\S]*max-width:\s*min\(100vw, 38rem\) !important/);
  ...
  assert.match(globalsSource, /\.nexid-account-backdrop\s*\{[\s\S]*rgba\(2, 6, 23, 0\.98\) !important/);
  ...
  assert.match(globalsSource, /\.nexid-account-context\s*\{[\s\S]*width:\s*min\(44rem, calc\(100vw - 42rem - 6vw\)\) !important/);
  ...
  assert.match(globalsSource, /nexid-account-dialog::backdrop/);
  ...
  assert.match(globalsSource, /\.nexid-account-layer\s*\{[\s\S]*box-shadow:\s*-44px 0 140px/);
  ...
  assert.doesNotMatch(globalsSource, /body\.nexid-account-menu-open \.nexid-crm-shell,\s*\.nexid-crm-shell\[data-account-menu-suppressed="true"\]\s*\{[^}]*display:\s*none !important/);
  assert.match(globalsSource, /body\.nexid-account-menu-open \.nexid-crm-shell,\s*\.nexid-crm-shell\[data-account-menu-suppressed="true"\]\s*\{[^}]*z-index:\s*0 !important/);
  assert.match(globalsSource, /body\.nexid-account-menu-open \.nexid-crm-shell,\s*\.nexid-crm-shell\[data-account-menu-suppressed="true"\]\s*\{[^}]*filter:\s*saturate\(0\.6\) brightness\(0\.28\) blur\(1px\) !important/);
  assert.match(globalsSource, /body\.nexid-account-menu-open \.nexid-crm-shell,\s*\.nexid-crm-shell\[data-account-menu-suppressed="true"\]\s*\{[^}]*opacity:\s*0\.12 !important/);
  assert.doesNotMatch(globalsSource, /body\.nexid-account-menu-open \.nexid-crm-shell,\s*\.nexid-crm-shell\[data-account-menu-suppressed="true"\]\s*\{[^}]*visibility:\s*hidden !important/);
  assert.match(globalsSource, /body\.nexid-account-menu-open \.nexid-crm-shell,\s*\.nexid-crm-shell\[data-account-menu-suppressed="true"\]\s*\{[^}]*contain:\s*none !important/);
  assert.match(globalsSource, /body\.nexid-account-menu-open \.nexid-crm-shell,\s*\.nexid-crm-shell\[data-account-menu-suppressed="true"\]\s*\{[^}]*user-select:\s*none !important/);
