...
const output=resolve(process.env.QA_OUTPUT||'artifacts/request-inbox-browser');await mkdir(output,{recursive:true});
...
const report={localOnly:true,syntheticData:true,actualComponents:['SupplierRequestWorkspace','SupplierRequestInbox'],actualClientContract:true,httpBoundaryMocked:true,navigationStubbed:true,realNextServer:false,realDatabase:false,productionTested:false,checks:[],views:[],clientErrors:[],blockedRequests:[]};
...
const fixedId=n=>'40000000-0000-4000-8000-'+String(n).padStart(12,'0');
const fixtureRows=()=>[
 item({id:fixedId(1),title:'Edición Única Malbec',notes:'private-search-must-not-index',updated_at:'2026-09-25T08:00:00Z'}),
 submitted({id:fixedId(2),title:'Proyecto para responder',review_summary:{state:'needs_information',revision:1,updated_at:'2026-09-25T09:00:00Z'},updated_at:'2026-09-25T08:00:00Z'}),
 submitted({id:fixedId(3),title:'Revisión pendiente',updated_at:'2026-09-25T12:00:00Z'}),
 submitted({id:fixedId(4),title:'Respuesta de empresa',review_summary:{state:'answered',revision:2,updated_at:'2026-09-25T11:00:00Z'},updated_at:'2026-09-25T10:00:00Z'}),
 submitted({id:fixedId(5),title:'Pedido preparado',status:'provisioned',order_id:'60000000-0000-4000-8000-000000000001'}),
 submitted({id:fixedId(6),title:'Revisión sin confirmar',review_summary:undefined}),
];
try{
 for(const theme of ['light','dark'])for(const width of [320,390,1440]){
  const rows=fixtureRows(),t=await scenario(width,theme,{rows});
  check(await t.page.getByTestId('supplier-request-open').count()===6,'All six loaded records displayed');check((await t.page.getByTestId('inbox-group-company').innerText()).includes('1'),'Company reply count reflects loaded review state');check((await t.page.getByTestId('inbox-group-nexid').innerText()).includes('2'),'NexID review count reflects loaded pending/answered');
  const requestsBefore=t.state.reads.length;
  await t.page.getByTestId('supplier-request-inbox-search').fill('UNICA qa-only');check(await t.page.getByTestId('supplier-request-open').count()===1,'Multiword accent-insensitive search combines title and company');
  await t.page.getByTestId('supplier-request-open').click();await t.page.waitForFunction(title=>document.querySelector('[data-testid="supplier-request-title"]')?.value===title,rows[0].title);check(await t.page.getByTestId('supplier-request-title').inputValue()===rows[0].title,'Row opens its exact company and request reference');
  check(await t.page.getByTestId('supplier-request-open').getAttribute('aria-current')==='true','Open record selection is explicit and accessible');
  await t.page.getByTestId('supplier-request-title').fill('Borrador que no se pierde');const afterOpen=t.state.reads.length;
  await t.page.getByTestId('inbox-group-company').click();await t.page.getByTestId('supplier-request-inbox-search').fill('');
  check(await t.page.getByTestId('supplier-request-open').count()===1,'Group button filters reply-needed rows');check(await t.page.getByTestId('request-inbox-open-outside').count()===1,'Hidden selection does not imply the editor was closed');check(await t.page.getByTestId('supplier-request-title').inputValue()==='Borrador que no se pierde','Filtering preserves a local draft');
  await t.page.getByRole('link',{name:'Ir al expediente abierto',exact:true}).click();check(await t.page.getByTestId('supplier-request-detail-heading').evaluate(n=>document.activeElement===n),'Jump to open editor restores focus without rereading or discarding draft');
  await inspect(t.page,'inbox-company-action',width,theme);
  await t.page.getByTestId('request-inbox-clear').click();check(await t.page.getByTestId('supplier-request-inbox-search').evaluate(n=>document.activeElement===n),'Clearing restores keyboard focus to search');check(await t.page.getByTestId('supplier-request-open').count()===6,'Clear removes both search and filters');
  await t.page.getByTestId('request-inbox-sort').selectOption('recent');check((await t.page.getByTestId('request-inbox-row').first().innerText()).includes('Revisión pendiente'),'Activity descending uses the most recent known timestamp');
  await t.page.getByTestId('request-inbox-sort').selectOption('review');check((await t.page.getByTestId('request-inbox-row').first().innerText()).includes('Revisión sin confirmar'),'Unconfirmed state is not silently treated as approved');
  await t.page.getByTestId('supplier-request-inbox-search').fill('private-search-must-not-index');check(await t.page.getByTestId('supplier-request-open').count()===0,'Notes do not become a search index');
  await inspect(t.page,'inbox-no-match',width,theme);await t.page.getByTestId('request-inbox-clear').click();
  check(t.state.reads.length===afterOpen&&t.state.writes.length===0&&t.state.orderWrites.length===0,'Sorting/filter/reset never call API, mutate drafts or prepare orders');check(requestsBefore===1,'Initial list is one bounded read');
  t.page.once('dialog',d=>d.dismiss());await t.page.getByTestId('supplier-request-open').last().click();check(await t.page.getByTestId('supplier-request-title').inputValue()==='Borrador que no se pierde','Cancelled navigation retains the editor');
  await t.context.close();
 }
 const empty=await scenario();check(await empty.page.getByTestId('inbox-group-all').count()===1&&await empty.page.getByTestId('supplier-request-open').count()===0,'Confirmed empty result alone may show zero counts');empty.state.mode='read-error';await empty.page.getByRole('button',{name:'Actualizar bandeja',exact:true}).click();await empty.page.getByRole('status').filter({hasText:'No se confirmó la bandeja'}).waitFor();check(!await empty.page.getByTestId('inbox-group-all').count()&&!await empty.page.getByTestId('supplier-request-inbox-count').count(),'An unavailable list displays neither zero counts nor stale metrics');await empty.context.close();
 const t=await scenario(1440,'dark',{access:{role:'super-admin',tenantSlug:null},rows:fixtureRows()});check(await t.page.getByTestId('supplier-request-open').count()===5,'Global source does not expose internal drafts');check(t.state.reads.length===1,'Global list stays bounded');t.state.truncated=true;await t.page.getByRole('button',{name:'Actualizar bandeja',exact:true}).click();await t.page.getByTestId('supplier-request-inbox-count').filter({hasText:'no es el total del historial'}).waitFor();await t.page.getByTestId('inbox-group-nexid').click();check(await t.page.getByTestId('supplier-request-open').count()===2,'Global NexID review group contains only records received');await inspect(t.page,'inbox-global-bounded',1440,'dark');await t.context.close();
 for(const patch of [{role:'viewer'},{isDemo:true},{deniedPermissions:['supplier_order.create']}]){const d=await scenario(390,'light',{access:patch,denied:true});check(d.state.reads.length===0&&!await d.page.getByTestId('supplier-request-inbox').count(),'Denied/demo access produces no query or totals');await d.context.close();}
