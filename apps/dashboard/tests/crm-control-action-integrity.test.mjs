...
const [crm, home] = await Promise.all([
...
test("CRM primary navigation uses client routing and the destination registry", () => {
  assert.match(crm, /import \{ DASHBOARD_DESTINATIONS, dashboardCanOpenDestination \}/);
  ...
  assert.match(crm, /router\.push\(DASHBOARD_DESTINATIONS\.campaigns\.href\)/);
  assert.match(crm, /router\.push\(`\$\{DASHBOARD_DESTINATIONS\.campaigns\.href\}\?\$\{params\.toString\(\)\}`\)/);
  assert.doesNotMatch(crm, /window\.location\.href\s*=\s*["'`]\/(?:events|loyalty\/campaigns)/);
});

test("restricted event and campaign actions render explanatory unavailable states", () => {
  assert.match(crm, /const eventsUnavailableReason = "Auditoría no habilitada: esta sesión no tiene events\.read_sensitive\."/);
  assert.match(crm, /const campaignsUnavailableReason = "Campañas no habilitadas: esta sesión no tiene campaigns:read\."/);
  assert.match(crm, /data-testid="events-audit-unavailable">\{eventsUnavailableReason\}<\/span>/);
  assert.match(crm, /data-testid="alerts-audit-unavailable">Auditoría no habilitada<\/span>/);
  assert.match(crm, /canOpenCampaigns \? \([\s\S]*?data-testid="opportunity-campaign-unavailable"/);
  assert.match(crm, /if \(!canOpenCampaigns\) return;/);
});
