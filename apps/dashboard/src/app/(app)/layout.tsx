      ...
      <div className="min-w-0 flex-1">
        <header className="border-b border-white/10 bg-slate-950/70 px-4 py-4 backdrop-blur-xl lg:px-8">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="text-xs uppercase tracking-[0.18em] text-cyan-300">{t.dashboard.subtitle}</div>
              <div className="mt-1 text-xl font-bold text-white">{t.dashboard.title}</div>
            </div>
            <div className="flex items-center gap-2">
              <LocaleSwitcher value={locale} options={[...locales]} />
              <Badge tone="green">{t.common.apiConnected}</Badge>
              <Link href="/login" className="rounded-lg border border-white/10 px-3 py-2 text-xs">
                {t.common.logout}
              </Link>
            </div>
          </div>
