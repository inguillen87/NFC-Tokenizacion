...
export default function CreateSupplierOrderPage() {
  ...
  const frozen = loading || uncertain || created;
        ...
        <fieldset disabled={frozen || !accessResolved || !canCreateSupplierOrder} className={styles.fields}>
          <section className={styles.card} aria-labelledby="supplier-order-basics">
            ...
            <div className={styles.grid}>
              ...
              <div className={styles.field}><label htmlFor="supplier-order-name">Nombre del pedido</label><input id="supplier-order-name" name="order_name" required autoComplete="off" value={draft.order_name} onChange={(event) => setField("order_name", event.target.value)} placeholder="Ej.: Ensayo de etiquetas para envases" /></div>
          ...
          <section className={styles.card} aria-labelledby="supplier-construction-heading">
            ...
            <div className={styles.applyRow}><div className={styles.field}><label htmlFor="supplier-construction">Construcción de etiqueta</label><select id="supplier-construction" data-testid="supplier-construction-select" value={constructionId} onChange={(event) => setConstructionId(event.target.value)}><option value="">Elegí una opción</option>{SUPPLIER_CONSTRUCTIONS.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></div><button type="button" className={styles.secondary} data-testid="supplier-construction-apply" disabled={!construction} onClick={() => { if (!construction || submitting.current || uncertain) return; setDraft((current) => applySupplierConstruction(current, construction.id)); setAppliedConstructionId(construction.id); }}>Usar esta configuración</button></div>
          ...
          <details className={styles.card}>
            ...
            <div className={styles.grid}>
              {draft.carrier_profile_code !== "uhf_rfid" ? <div className={styles.field}><label htmlFor="supplier-order-chip-model">Modelo del chip</label><input id="supplier-order-chip-model" name="chip_model" aria-required="true" autoComplete="off" value={draft.chip_model} onChange={(event) => setField("chip_model", event.target.value)} /></div> : null}
              <div className={styles.field}><label htmlFor="supplier-order-carrier-profile">Perfil de identificación</label><select id="supplier-order-carrier-profile" name="carrier_profile_code" value={draft.carrier_profile_code} onChange={(event) => setField("carrier_profile_code", event.target.value)}><option value="">Sin configurar</option><option value="ntag424_dna">NFC seguro · NTAG424 DNA</option><option value="ntag424_dna_tt">NFC con detección de apertura · DNA TagTamper</option><option value="uhf_rfid">UHF · logística</option><option value="ntag213">NFC estático · NTAG213</option><option value="gs1_digital_link">GS1 Digital Link</option><option value="event_wristband">Pulsera de eventos</option></select></div>
              <div className={styles.field}><label htmlFor="supplier-order-material-type">Material o construcción</label><input id="supplier-order-material-type" name="material_type" autoComplete="off" value={draft.material_type} onChange={(event) => setField("material_type", event.target.value)} /></div>
          ...
          <section className={styles.card} aria-labelledby="supplier-order-review">
            ...
            <div className={styles.field}><label htmlFor="supplier-order-notes">Notas para preparar el pedido (opcional)</label><textarea id="supplier-order-notes" name="notes" rows={3} value={draft.notes} onChange={(event) => setField("notes", event.target.value)} /><p>No incluyas llaves ni secretos. Las especificaciones de rollo, embalaje y evidencias se completan después en sus secciones.</p></div>
