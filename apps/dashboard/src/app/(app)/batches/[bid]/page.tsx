...
{dashboardSessionCanOpenDestination(session,"logistics")&&<section className={styles.card}><h2>Trazabilidad del lote</h2><Link prefetch={false} className={styles.button} href={`/batches/${encodeURIComponent(bid)}/traceability?${new URLSearchParams({tenant:dossier.tenant})}`}>Abrir trazabilidad</Link></section>}
