...
import {readTraceJson} from '../../../../../lib/batch-traceability';
import {parseTracePage,type TracePage} from '../../../../../lib/trace-page';
...
export default async function TraceabilityPage({params,searchParams}:{params:Promise<{bid:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
 const session=await requireDashboardSession('batches:read'),{bid}=await params,query=await searchParams,context=await createAdminPageContext(session,query.tenant);let data:TracePage|null=null;
 ...
 return <><BatchWorkspaceNavigationServer bid={bid} tenant={context.tenantSlug} current="traceability" session={session}/><BatchTraceabilityWorkspace key={`${session.id}:${context.tenantSlug}:${bid}`} initial={data} bid={bid} tenant={context.tenantSlug} canLogistics={dashboardSessionCanOpenDestination(session,'logistics')} enabled={!session.isDemo} canIntake={canUseEpcisIntake(session)}/></>;
}
