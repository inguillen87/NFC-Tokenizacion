...
export default async function ChannelsPage({params,searchParams}:{params:Promise<{bid:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <BatchChannelWorkspace key={`${session.id}:${context.tenantSlug}:${bid}`} initial={data} bid={bid} tenant={context.tenantSlug} canOrder={dashboardSessionCanOpenDestination(session,'supplierBatches')}/>;
}
