...
export default async function IntakePage({params,searchParams}:{params:Promise<{bid:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <><BatchWorkspaceNavigationServer bid={bid} tenant={context.tenantSlug} current="intake" session={session}/><EpcisIntakeWorkspace key={`${session.id}:${context.tenantSlug}:${bid}`} initial={data} tenant={context.tenantSlug} bid={bid}/></>;
}
