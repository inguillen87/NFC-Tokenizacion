...
export default async function BatchProductionPage({params,searchParams}:{params:Promise<{bid:string}>;searchParams:Promise<Record<string,string|string[]|undefined>>}){
 ...
 return <><BatchWorkspaceNavigationServer bid={bid} tenant={context.tenantSlug} current="production" session={session}/><LabelProductionWorkspace key={`${session.id}:${context.tenantSlug}:${bid}`} initial={data} bid={bid} tenant={context.tenantSlug}/></>;
}
