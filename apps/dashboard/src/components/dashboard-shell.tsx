...
export function DashboardShell({
  ...
  const items = useMemo(
    () =>
      roleAccess[role]
        .map((key) => ({ key, href: key === "overview" ? "/" : `/${key === "apiKeys" ? "api-keys" : key}`, label: nav[key as NavKey] }))
    ...
    <div className="min-h-screen bg-slate-950 text-white lg:flex">
      <aside className="hidden w-72 shrink-0 border-r border-white/10 bg-slate-950/80 p-5 lg:block">
        ...
        <div className="mt-5 space-y-3">
          ...
          <select className="w-full rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm" value={role} onChange={(event) => setRole(event.target.value as DashboardRole)}>
