...
export function DataTable({
  ...
  loadingLabel: string;
  emptyLabel: string;
  labels: { search: string; all: string; refresh: string };
}) {
    ...
    <Card className="p-5">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <input placeholder={labels.search} value={query} onChange={(event) => setQuery(event.target.value)} className="rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-sm" />
          ...
          <Button variant="secondary" onClick={async () => { setLoading(true); await new Promise((r) => setTimeout(r, 500)); setLoading(false); }}>{labels.refresh}</Button>
        </div>
      </div>
