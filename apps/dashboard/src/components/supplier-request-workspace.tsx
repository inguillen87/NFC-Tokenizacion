...
function RequestWorkspace({ access, initialTenant = "", initialRequestId = "" }: Props) {
  ...
  const canReviewInteract = useCallback(() => !reads.current && !locked.current && !assignmentGuard.current.locked && !cancellationGuard.current.locked && !quotationGuard.current.locked, []);
  const canAssignmentInteract = useCallback(() => !reads.current && !locked.current && !reviewGuard.current.locked && !cancellationGuard.current.locked && !quotationGuard.current.locked, []);
  const dirty = JSON.stringify(fields) !== JSON.stringify(selected ? fromItem(selected) : empty());
  const immutable = Boolean(selected && selected.status !== "draft"), blocked = phase === "saving" || phase === "uncertain" || unresolved.current || reviewBlocked || assignmentBlocked || cancellationBlocked || quotationBlocked || quotationAccessLost;
  ...
  useEffect(() => {
    const warn = (event: BeforeUnloadEvent) => { if (dirty || locked.current || reviewGuard.current.dirty || reviewGuard.current.locked || assignmentGuard.current.dirty || assignmentGuard.current.locked || cancellationGuard.current.locked || quotationGuard.current.locked) { event.preventDefault(); event.returnValue = ""; } };
  ...
  async function loadList(scope: string) {
    if (locked.current || reviewGuard.current.locked || assignmentGuard.current.locked || cancellationGuard.current.locked || quotationGuard.current.locked) return;
  ...
  function canLeave() {
    if (locked.current || reviewGuard.current.locked || assignmentGuard.current.locked || cancellationGuard.current.locked || quotationGuard.current.locked) return false;
  ...
  function resetEditor() { updateQuotationGuard({dirty:false,locked:false});setQuotationAccessLost(false); updateCancellationGuard({ dirty: false, locked: false }); updateReviewGuard({ dirty: false, locked: false }); updateAssignmentGuard({ dirty: false, locked: false }); setSelected(null); setFields(empty()); setComparison(null); setPhase("idle"); setError(""); setConfirmedRevision(null); setReviewing(false); setRequestReading(false); operation.current = null; }
  ...
  async function openRequest(id: string, scope: string, initial = false, compare = false) {
    if (!allowed || reviewGuard.current.locked || assignmentGuard.current.locked || cancellationGuard.current.locked || quotationGuard.current.locked || (!initial && !compare && !canLeave()) || busy.current || (locked.current && !compare)) return;
  ...
  async function execute(command: SupplierRequestCommand) {
    if (!allowed || busy.current || cancellationGuard.current.locked || quotationGuard.current.locked) return;
  ...
  return <main className={styles.root} data-testid="supplier-request-workspace">
    ...
    {!allowed ? <p role="alert" className={styles.notice}>Tu sesión no tiene permiso para gestionar solicitudes. No se consultaron ni guardaron datos.</p> : <>
      ...
      {(tenant || selected) ? <section className={styles.card} aria-labelledby="supplier-request-editor">
        ...
        {selected && immutable && selected.status !== "cancelled" && canAssign ? <SupplierRequestAssignment key={`assignment:${selected.id}:${selected.revision}:${selected.status}:${selectionVersion}`} request={selected} canManageUsers={dashboardPermissionMatches(access.permissions, "users:manage", access.deniedPermissions)} suspended={requestReading || reviewBlocked || cancellationBlocked || quotationBlocked} canInteract={canAssignmentInteract} onGuard={updateAssignmentGuard} /> : null}
        {selected && immutable ? <SupplierRequestReview key={`${selected.id}:${selected.revision}:${selected.status}:${selectionVersion}`} request={selected} isNexid={isNexid} suspended={requestReading || assignmentBlocked || cancellationBlocked || quotationBlocked} canInteract={canReviewInteract} onGuard={updateReviewGuard} onReview={updateReview} /> : null}
        </> : <h2 id="supplier-request-editor">Acceso a la solicitud no disponible</h2>}
        {selected && immutable ? <SupplierRequestQuotation key={`quote:${selected.id}:${selectionVersion}`} request={selected} isNexid={isNexid} suspended={reading||blocked}
          canInteract={()=>!reads.current&&!locked.current&&!reviewGuard.current.locked&&!reviewGuard.current.dirty&&!assignmentGuard.current.locked&&!assignmentGuard.current.dirty&&!cancellationGuard.current.locked}
        ...
        {!quotationAccessLost ? <>
        {selected?.status === "submitted" ? <SupplierRequestCancellation key={`cancel:${selected.id}:${selected.revision}:${selectionVersion}`} request={selected} suspended={reading || blocked} canInteract={() => !reads.current && !locked.current && !reviewGuard.current.locked && !reviewGuard.current.dirty && !assignmentGuard.current.locked && !assignmentGuard.current.dirty && !quotationGuard.current.locked} onGuard={updateCancellationGuard} onUncertainExit={() => { resetEditor(); void loadList(tenant); }} onCurrent={item => { if (item.id !== selected.id || item.tenant_id !== selected.tenant_id) return; updateCancellationGuard({ dirty: false, locked: false }); updateReviewGuard({ dirty: false, locked: false }); updateAssignmentGuard({ dirty: false, locked: false }); setSelected(item); setFields(fromItem(item)); setSelectionVersion(value => value + 1); setItems(current => current?.map(row => row.id === item.id && row.tenant_id === item.tenant_id ? item : row) || current); }} /> : null}
