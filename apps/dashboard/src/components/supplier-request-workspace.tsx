...
import { supplierRequestCall, supplierRequestErrorCopy, SupplierRequestError, type SupplierRequest, type SupplierRequestCommand, type SupplierRequestContent, type SupplierRequestEnvelope } from "../lib/supplier-request-client";
...
function RequestWorkspace({ access, initialTenant = "", initialRequestId = "" }: Props) {
  ...
  const immutable = Boolean(selected && selected.status !== "draft"), blocked = phase === "saving" || phase === "uncertain" || unresolved.current;

  useEffect(() => { alive.current = true; return () => { alive.current = false; epoch.current++; reads.current?.abort(); writes.current?.abort(); }; }, []);
  useEffect(() => {
    const warn = (event: BeforeUnloadEvent) => { if (dirty || locked.current) { event.preventDefault(); event.returnValue = ""; } };
  ...
  function canLeave() {
    ...
    return !dirty || window.confirm("Hay cambios sin guardar. ¿Descartarlos y continuar?");
  }
  function resetEditor() { setSelected(null); setFields(empty()); setComparison(null); setPhase("idle"); setError(""); setConfirmedRevision(null); setReviewing(false); setRequestReading(false); operation.current = null; }
  ...
  async function openRequest(id: string, scope: string, initial = false, compare = false) {
    if (!allowed || (!initial && !compare && !canLeave()) || busy.current || (locked.current && !compare)) return;
    ...
    try {
      ...
      else { setSelected(result.request); setFields(fromItem(result.request)); setComparison(null); setPhase("idle"); setConfirmedRevision(null); setReviewing(false); }
  ...
  return <main className={styles.root} data-testid="supplier-request-workspace">
    ...
    {!allowed ? <p role="alert" className={styles.notice}>Tu sesión no tiene permiso para gestionar solicitudes. No se consultaron ni guardaron datos.</p> : <>
      ...
      <section className={styles.card} aria-labelledby="supplier-requests-inbox"><div className={styles.heading}><h2 id="supplier-requests-inbox">{isNexid && !tenant ? "Bandeja de NexID" : "Solicitudes de la empresa"}</h2><div className={styles.actions}><button className={styles.button} type="button" disabled={blocked || reading} onClick={() => { if (!locked.current) void loadList(tenant); }}>Actualizar bandeja</button><button className={styles.button} type="button" disabled={blocked || !tenant} onClick={() => { if (canLeave()) { epoch.current++; reads.current?.abort(); resetEditor(); } }}>Nueva solicitud</button></div></div>
        ...
        <p role="status">{listError || (reading && items === null ? "Consultando la fuente…" : items?.length === 0 ? "La fuente no encontró solicitudes en este alcance." : "")}</p>
        {items?.length ? <ul className={styles.list}>{items.map(item => <li key={item.id}><div><strong>{item.title}</strong><span>{item.tenant_slug} · {statusLabel(item.status)} · versión {item.revision}</span><code>{item.id}</code></div><button className={styles.button} data-testid="supplier-request-open" type="button" disabled={blocked || reading} onClick={() => void openRequest(item.id, item.tenant_slug)}>Ver solicitud</button></li>)}</ul> : null}
      ...
      {(tenant || selected) ? <section className={styles.card} aria-labelledby="supplier-request-editor">
        <div className={styles.heading}><div><h2 id="supplier-request-editor">{selected ? selected.title : "Nueva solicitud"}</h2><p className={styles.muted}>{currentTenant}{selected ? ` · ${statusLabel(selected.status)} · ${selected.id}` : " · todavía sin guardar"}</p></div></div>
        ...
        {isNexid && selected?.status === "submitted" ? <a className={styles.primary} data-testid="supplier-request-prepare" href={`/supplier-orders/create?request=${selected.id}&tenant=${encodeURIComponent(selected.tenant_slug)}`}>Preparar pedido técnico</a> : null}
