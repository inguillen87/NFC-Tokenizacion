...
import type { DashboardCopy, DashboardRole } from "../i18n/types";

export function AdminActionForms({ copy, roles }: { copy: DashboardCopy["forms"]; roles: DashboardCopy["roles"] }) {
  const [role, setRole] = useState<DashboardRole>("super-admin");
  ...
  async function submit(path: string, payload: unknown) {
    ...
    try {
      ...
      setStatus(error instanceof Error ? error.message : copy.apiStatus);
    }
  }
  ...
  return (
    <div className="space-y-6">
      <Card className="p-5">
        ...
        <select className="mt-2 rounded-xl border border-white/10 bg-slate-950 px-3 py-2 text-sm" value={role} onChange={(event) => setRole(event.target.value as DashboardRole)}>
      ...
      <div className="grid gap-6 xl:grid-cols-2">
        <Card className="p-5">
          <h3 className="text-base font-semibold text-white">{copy.createTenant}</h3>
          <div className="mt-4 grid gap-3">
            ...
            <Button disabled={!canEdit} onClick={() => submit("/admin/tenants/", tenant)}>{copy.actions.createTenant}</Button>
          </div>
        </Card>

        <Card className="p-5">
          <h3 className="text-base font-semibold text-white">{copy.createBatch}</h3>
          <div className="mt-4 grid gap-3">
            ...
            <Button disabled={!canEdit} onClick={() => submit("/admin/batches/", { ...batch, quantity: Number(batch.quantity || 0) })}>{copy.actions.createBatch}</Button>
          </div>
        </Card>

        <Card className="p-5">
          <h3 className="text-base font-semibold text-white">{copy.importManifest}</h3>
          <div className="mt-4 grid gap-3">
            ...
            <Button disabled={!canEdit} onClick={() => submit(`/admin/batches/${manifest.batchId}/import-manifest/`, { csv: manifest.csv })}>{copy.actions.importManifest}</Button>
          </div>
        </Card>

        <Card className="p-5">
          <h3 className="text-base font-semibold text-white">{copy.activateRevoke}</h3>
          <div className="mt-4 grid gap-3">
