...
type Props = { request: SupplierRequest; isNexid: boolean; suspended: boolean; canInteract: () => boolean; onGuard: (guard: SupplierRequestReviewGuard) => void; onReview: (summary: SupplierRequestReviewSummary) => void };
...
export function SupplierRequestReview({ request, isNexid, suspended, canInteract, onGuard, onReview }: Props) {
  ...
  const canAct = Boolean(currentRequest && request.status === "submitted" && data && (isNexid ? data.review.state !== "needs_information" : data.review.state === "needs_information"));
  ...
  async function refresh(older = false) {
    ...
    try {
      const result = await supplierRequestReviewCall({ tenant: request.tenant_slug, tenantId: request.tenant_id, id: request.id, ...(older ? { beforeRevision: data!.next_before_revision! } : {}), signal: controller.signal });
  ...
  async function execute(operation: SupplierRequestReviewCommand) {
    ...
    try {
      const result = await supplierRequestReviewCall({ tenant: request.tenant_slug, tenantId: request.tenant_id, id: request.id, command: operation, signal: controller.signal });
  ...
  function send() {
    ...
    void execute(Object.freeze({ tenant: request.tenant_slug, id: request.id, key: crypto.randomUUID(), body: Object.freeze({ action, message: value, expected_revision: data.review.revision, expected_request_revision: request.revision }) }));
  }
  ...
  return <section className={styles.reviewPanel} aria-labelledby="supplier-review-heading" data-testid="supplier-request-review-panel">
    ...
    <p className={styles.notice} role="status" data-testid="supplier-request-review-state">{reading ? "Consultando aclaraciones…" : phase === "saving" ? "Guardando el mensaje…" : phase === "uncertain" ? "Mensaje sin confirmar. La misma operación sigue pendiente." : !data || phase === "error" ? "No se confirmó el estado de revisión. La preparación técnica sigue bloqueada." : !currentRequest ? "La solicitud cambió. Volvé a abrir su versión actual; tu texto se conserva hasta que decidas salir." : request.status === "provisioned" ? "Pedido preparado: historial disponible para consulta." : stateCopy[data.review.state]}</p>
    ...
    {isNexid && request.status === "submitted" ? confirmedCurrent && data!.review.state !== "needs_information" && !message.trim() && !blocked ? <a className={styles.primary} data-testid="supplier-request-prepare" href={`/supplier-orders/create?request=${request.id}&tenant=${encodeURIComponent(request.tenant_slug)}`} onClick={event => { if (busy.current || unresolved.current || text.current.trim() || readingRef.current) event.preventDefault(); }}>Preparar pedido técnico</a> : <p className={styles.muted} data-testid="supplier-request-prepare-blocked">{data?.review.state === "needs_information" ? "Preparación bloqueada: falta la respuesta de la empresa." : message.trim() ? "Revisá o descartá el mensaje escrito antes de preparar el pedido." : "La preparación requiere un estado de revisión confirmado."}</p> : null}
  </section>;
}
