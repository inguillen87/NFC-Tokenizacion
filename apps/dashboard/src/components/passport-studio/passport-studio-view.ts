...
export function mountPassportStudio(host:HTMLElement,port:StudioPort){
 ...
 function renderSection(){
  ...
  if(['identity','agro','documents'].includes(group)){
   ...
   el('fields').innerHTML=`<div class="ps-change-summary"><strong>${diffs.length}</strong><span>campos diferentes</span><small>${snapshot.published?`${snapshot.published.version>0?`Respecto de la versión ${snapshot.published.version}`:'Respecto del contenido inicial sin revisión'}`:'Todavía no hay una versión publicada'}</small></div>${issues.length?`<div class="ps-issue-list">${issues.map(i=>`<button type="button" data-error-field="${i.field}" class="ps-issue ${i.severity==='error'?'is-error':''}">${icon('warning')}<span><b>${esc(i.label)}</b>${esc(i.message)}</span></button>`).join('')}</div>`:''}<div class="ps-diff-list">${diffs.map(d=>`<article class="ps-diff"><h3>${esc(d.label)}</h3><div><section><span>PUBLICADO</span><p>${esc(d.before||'Sin contenido')}</p></section><section><span>BORRADOR</span><p>${esc(d.after||'Se elimina el contenido')}</p></section></div></article>`).join('')||'<p class="ps-empty">No hay diferencias con lo publicado.</p>'}</div>`;
  }else{
   el('fields').innerHTML=`<ol class="ps-history">${snapshot.history.map(h=>`<li><span class="ps-history-dot"></span><div><span class="ps-chip">r${h.version} · ${esc(actionText[h.action as StudioAction]||(h.action==='restore'?'Nueva revisión':'Versión anterior'))}</span><h3>${esc(h.actorLabel)}</h3><time>${esc(new Date(h.at).toLocaleString('es-AR',{timeZone:'UTC'}))} UTC</time><p>${esc(h.note||'')}</p><p>${esc(h.document.identity.product_name||'Producto sin nombre')}</p><button type="button" class="ps-link" data-restore="${h.id}" ${!editable()?'disabled':''}>Usar contenido en el borrador</button></div></li>`).join('')||'<li class="ps-empty">La fuente no entregó versiones anteriores.</li>'}</ol>`;
  }
 }
