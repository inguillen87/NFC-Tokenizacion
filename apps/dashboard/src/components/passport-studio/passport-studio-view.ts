...
const actionText:Record<StudioAction,string>={save:'Guardar borrador',submit:'Enviar a revisión',request_changes:'Solicitar cambios',approve:'Aprobar contenido',publish:'Publicar esta versión',reopen:'Preparar nueva revisión'};
...
export function mountPassportStudio(host:HTMLElement,port:StudioPort){
 ...
 function chrome(){root.innerHTML=`
  ...
  <aside class="ps-preview-area" aria-label="Vista previa del contenido"><div class="ps-preview-heading"><span>${icon('eye')}Vista móvil</span><span class="ps-chip ps-preview-badge">VISTA PREVIA</span></div><div class="ps-preview-switch" role="group" aria-label="Contenido a previsualizar"><button type="button" data-preview="draft" aria-pressed="true">Borrador</button><button type="button" data-preview="published" aria-pressed="false" ${snapshot.published?'':'disabled'}>${snapshot.published?.version===0?'Contenido inicial':'Publicado'}${snapshot.published&&snapshot.published.version>0?` · v${snapshot.published.version}`:''}</button></div><div class="ps-phone" data-slot="phone" tabindex="0" role="region" aria-label="Vista previa del pasaporte desplazable"></div><p class="ps-preview-foot">La vista previa no verifica una etiqueta NFC. No representa una lectura real ni una certificación.</p></aside></div>
  ...
  <dialog class="ps-dialog" aria-labelledby="ps-confirm-title"><form method="dialog"><button class="ps-dialog-close" aria-label="Cerrar confirmación" value="cancel">${icon('close')}</button><span class="ps-dialog-icon">${icon('files')}</span><h2 id="ps-confirm-title"></h2><p data-slot="confirm-detail"></p><div data-slot="confirm-comparison" class="ps-confirm-comparison" hidden></div><label class="ps-note-label" hidden>Comentario de revisión<textarea data-review-note maxlength="800" rows="3"></textarea></label><div class="ps-dialog-actions"><button class="ps-btn" value="cancel">Cancelar</button><button class="ps-btn ps-primary" value="confirm">Confirmar</button></div></form></dialog>`;renderSection();refresh();}
 ...
 function refresh(){
  ...
  el('validation').innerHTML=(uncertain?`<div class="ps-notice ps-alert">${icon('warning')}<div><strong>No se confirmó la operación.</strong><p>Conservamos los datos y el mismo identificador. No crees otro intento hasta reconciliar la respuesta.</p><button type="button" class="ps-link" data-do="retry" ${busy?'disabled':''}>Reintentar el mismo intento</button></div></div>`:conflict?`<div class="ps-notice ps-alert">${icon('warning')}<div><strong>Existe una revisión más reciente.</strong><p>Tus cambios siguen en esta pestaña. No se sobrescribió la versión guardada. Releé la versión guardada antes de continuar; requiere descartar explícitamente los cambios locales.</p><button type="button" class="ps-link" data-do="reload">Releer versión guardada</button><p></p></div></div>`:!editable()?`<div class="ps-notice">${icon('lock')}<div><strong>${snapshot.draft.state==='published'?'Versión publicada · solo consulta':snapshot.draft.state==='in_review'?'Contenido en revisión':'Edición no disponible'}</strong><p>${snapshot.draft.state==='in_review'?'La revisión usa el contenido guardado. Solo un revisor autorizado e independiente puede aprobarlo.':'El estado y los permisos se confirman en el servidor.'}</p></div></div>`:'');
  const actions=(['submit','request_changes','approve','publish','reopen'] as StudioAction[]).filter(a=>canStudioAction(snapshot,a,changed));
  ...
  const diff=compareEditorialDocuments(snapshot.published?.document||null,form,{after:true});const count=root.querySelector('[data-count="changes"]');if(count)count.textContent=String(diff.length);
  for(const input of root.querySelectorAll<HTMLInputElement|HTMLTextAreaElement>('[data-field]'))input.disabled=!editable();
  const publishedTab=root.querySelector<HTMLButtonElement>('[data-preview="published"]');
  if(publishedTab){publishedTab.disabled=!snapshot.published;publishedTab.textContent=snapshot.published?.version===0?'Contenido inicial':snapshot.published?`Publicado · v${snapshot.published.version}`:'Publicado';}
  renderPreview();
 }
 function selectedComparison(){
  const sources=comparisonSources(snapshot,form);
  if(!sources.some(x=>x.key===compareBase))compareBase='published';
  if(!sources.some(x=>x.key===compareTarget))compareTarget='working';
  const before=comparisonSource(sources,compareBase),after=comparisonSource(sources,compareTarget);
  return {sources,before,after,rows:compareEditorialDocuments(before.document,after.document,{before:before.key==='working',after:after.key==='working'})};
 }
 function renderComparison(){
  const {sources,before,after,rows}=selectedComparison(),issues=reviewIssues(form);
  const options=(selected:string)=>sources.map(x=>`<option value="${esc(x.key)}" ${x.key===selected?'selected':''}>${esc(x.label)}</option>`).join('');
  el('fields').innerHTML=`<div class="ps-compare-sources"><label>Documento base<select data-comparison-source="base" aria-label="Documento base de comparación">${options(compareBase)}</select></label><button type="button" class="ps-btn ps-compare-swap" data-do="swap-comparison" aria-label="Intercambiar documentos comparados">${icon('changes')}</button><label>Comparar con<select data-comparison-source="target" aria-label="Documento de destino de comparación">${options(compareTarget)}</select></label></div>
   <div class="ps-compare-context"><section><strong>${esc(before.label)}</strong><p>${esc(before.detail)}</p></section><section><strong>${esc(after.label)}</strong><p>${esc(after.detail)}</p></section></div>
   <div class="ps-comparison-counts">${(['added','changed','removed'] as const).map(kind=>`<div data-kind="${kind}"><strong>${rows.filter(d=>d.kind===kind).length}</strong><span>${kind==='added'?'Campos añadidos':kind==='removed'?'Campos retirados':'Campos modificados'}</span></div>`).join('')}</div>
   ${before.local||after.local?'<div class="ps-notice"><div><strong>Incluye trabajo local sin guardar.</strong><p>La comparación no constituye una revisión aprobada. Guardá antes de enviar a revisión.</p></div></div>':''}
   <div class="ps-compare-filters"><label>Sección<select data-comparison-filter="group" aria-label="Filtrar diferencias por sección"><option value="all">Todas las secciones</option>${Object.entries(COMPARISON_GROUPS).map(([k,v])=>`<option value="${k}" ${compareGroup===k?'selected':''}>${esc(v)}</option>`).join('')}</select></label><label>Tipo de cambio<select data-comparison-filter="kind" aria-label="Filtrar diferencias por tipo"><option value="all">Todos los cambios</option>${Object.entries(COMPARISON_KINDS).map(([k,v])=>`<option value="${k}" ${compareKind===k?'selected':''}>${v}</option>`).join('')}</select></label><label>Buscar en los cambios<input type="search" data-comparison-search maxlength="160" value="${esc(compareSearch)}" placeholder="Campo o contenido" aria-label="Buscar en las diferencias"/></label></div>
   <p class="ps-compare-result" data-slot="comparison-result" role="status" aria-live="polite"></p><div class="ps-diff-list" data-slot="comparison-rows"></div>
   <p class="ps-compare-scope">Sólo documentos del contrato editorial recibidos en esta consulta. Una URL distinta no demuestra que su archivo haya cambiado; no se descargan documentos ni imágenes. Las revisiones históricas no equivalen a versiones publicadas.</p>
   ${issues.length?`<details class="ps-compare-issues"><summary>Revisiones pendientes del trabajo actual · ${issues.length}</summary><div class="ps-issue-list">${issues.map(i=>`<button type="button" data-error-field="${i.field}" class="ps-issue ${i.severity==='error'?'is-error':''}">${icon('warning')}<span><b>${esc(i.label)}</b>${esc(i.message)}</span></button>`).join('')}</div></details>`:''}`;
  renderComparisonRows();
 }
 function renderComparisonRows(){
  const {before,after,rows}=selectedComparison(),visible=filterEditorialDifferences(rows,compareGroup,compareKind,compareSearch);
  el('comparison-result').textContent=`${visible.length} de ${rows.length} diferencias visibles. Filtros locales: no generan consultas.`;
  el('comparison-rows').innerHTML=visible.map(d=>`<article class="ps-diff" data-diff-field="${esc(d.field)}"><header><h3>${esc(d.label)}</h3><span class="ps-diff-kind" data-kind="${d.kind}">${COMPARISON_KINDS[d.kind]}</span></header><small class="ps-diff-section">${esc(COMPARISON_GROUPS[d.group])}</small><div><section><span>${esc(before.label)}</span><p>${esc(d.before||'Sin contenido en esta fuente')}</p></section><section><span>${esc(after.label)}</span><p>${esc(d.after||'Sin contenido en esta fuente')}</p></section></div>${d.link?'<p class="ps-diff-link-note">Se compara el enlace declarado, no el contenido del archivo externo.</p>':''}</article>`).join('')||`<div class="ps-empty"><strong>${rows.length?'No hay cambios que coincidan con estos filtros.':'Los documentos comparados no tienen diferencias editoriales.'}</strong><p>${rows.length?'Cambiá o limpiá los filtros para ver los demás campos.':'Esto no verifica su exactitud, vigencia ni aprobación.'}</p>${rows.length?'<button type="button" class="ps-link" data-do="clear-comparison-filters">Mostrar todas las diferencias</button>':''}</div>`;
 }
 function confirmationComparison(before:StudioDocument|null,after:StudioDocument|null,caption:string){
  const rows=compareEditorialDocuments(before,after,{before:before===form,after:after===form}),box=el('confirm-comparison');box.hidden=false;
  box.innerHTML=`<strong>${esc(caption)}</strong><p>${rows.length} campos cambian en el contenido editorial.</p>${rows.length?`<ul>${rows.slice(0,6).map(d=>`<li>${esc(d.label)} · ${COMPARISON_KINDS[d.kind].toLowerCase()}</li>`).join('')}</ul>${rows.length>6?`<small>Y ${rows.length-6} campos más; revisalos en Cambios.</small>`:''}`:'<small>No se detectaron diferencias de campos entre estas dos fuentes.</small>'}`;
 }
 function renderSection(){
  root.classList.toggle('ps-comparing',group==='changes');
  ...
  }else if(group==='changes'){renderComparison();
  }else{
   el('fields').innerHTML=`<ol class="ps-history">${snapshot.history.map(h=>`<li><span class="ps-history-dot"></span><div><span class="ps-chip">r${h.version} · ${esc(actionText[h.action as StudioAction]||(h.action==='restore'?'Nueva revisión':'Versión anterior'))}</span><h3>${esc(h.actorLabel)}</h3><time>${esc(new Date(h.at).toLocaleString('es-AR',{timeZone:'UTC'}))} UTC</time><p>${esc(h.note||'')}</p><p>${esc(h.document.identity.product_name||'Producto sin nombre')}</p><div class="ps-history-actions"><button type="button" class="ps-link" data-history-compare="${h.id}">Comparar con el trabajo actual</button><button type="button" class="ps-link" data-restore="${h.id}" ${!editable()?'disabled':''}>Usar contenido en el borrador</button></div></div></li>`).join('')||'<li class="ps-empty">La fuente no entregó versiones anteriores.</li>'}</ol>`;
  }
 }
 ...
 function openConfirm(action:StudioAction){
  ...
  confirmAction=action;restoreId=null;reloadRequested=false;const dialog=root.querySelector<HTMLDialogElement>('dialog')!;
  root.querySelector('#ps-confirm-title')!.textContent=actionText[action];el('confirm-detail').textContent=action==='publish'?'Esta acción cambia el contenido visible del lote. El servidor debe verificar revisión, aprobación independiente y versión publicada antes de guardar.':action==='reopen'?'Se preparará otra revisión del contenido. La aprobación anterior no se reutiliza y el pasaporte publicado se conserva.':action==='save'?'Se guardará una revisión del borrador. No modifica el pasaporte publicado ni las etiquetas.':action==='submit'?'Se enviará el contenido guardado a revisión. Mientras se revisa, el editor queda en modo de consulta.':action==='approve'?'La aprobación corresponde al contenido guardado de esta revisión. No certifica autenticidad NFC ni condiciones físicas.':'La revisión vuelve al editor. Indicá qué necesita corregirse.';
  const note=root.querySelector<HTMLElement>('.ps-note-label')!;note.hidden=action!=='request_changes';root.querySelector<HTMLTextAreaElement>('[data-review-note]')!.value='';el('confirm-comparison').hidden=true;
  if(['submit','approve','publish'].includes(action))confirmationComparison(snapshot.published?.document||null,snapshot.draft.document,`Trabajo guardado r${snapshot.draft.revision} frente a ${snapshot.published?.version===0?'contenido inicial':snapshot.published?'publicado v'+snapshot.published.version:'ausencia de versión publicada'}`);
  dialog.returnValue='';dialog.showModal();
 }
 root.addEventListener('input',event=>{
  const target=event.target;if(target instanceof HTMLInputElement&&target.hasAttribute('data-comparison-search')){compareSearch=target.value;renderComparisonRows();return;}if(!(target instanceof HTMLInputElement||target instanceof HTMLTextAreaElement)||!target.dataset.field||!editable())return;
 ...
 root.addEventListener('change',event=>{
  ...
  if(target.dataset.comparisonSource){if(target.dataset.comparisonSource==='base')compareBase=target.value;else compareTarget=target.value;const selector=target.dataset.comparisonSource;renderComparison();root.querySelector<HTMLElement>(`[data-comparison-source="${selector}"]`)?.focus();return;}
  if(target.dataset.comparisonFilter){if(target.dataset.comparisonFilter==='group')compareGroup=target.value;else compareKind=target.value;renderComparisonRows();}
 },{signal});
 root.addEventListener('click',event=>{
  ...
  if(target.dataset.restore&&editable()){
   const chosen=snapshot.history.find(h=>h.id===target.dataset.restore);if(!chosen)return;
   restoreId=chosen.id;confirmAction=null;reloadRequested=false;root.querySelector('#ps-confirm-title')!.textContent='Recuperar contenido en el borrador';el('confirm-detail').textContent=`Se usará la revisión histórica r${chosen.version}, de ${chosen.actorLabel}. Reemplaza el trabajo local de esta pestaña; no guarda, aprueba ni publica.`;
   confirmationComparison(form,chosen.document,'Cambios sobre tu trabajo actual');root.querySelector<HTMLElement>('.ps-note-label')!.hidden=true;const dialog=root.querySelector<HTMLDialogElement>('dialog')!;dialog.returnValue='';dialog.showModal();return;
  }
  ...
  if(target.dataset.do==='reload'&&conflict&&!busy&&!uncertain){reloadRequested=true;restoreId=null;confirmAction=null;root.querySelector('#ps-confirm-title')!.textContent='Releer versión guardada';el('confirm-detail').textContent='Se volverá a consultar el documento del servidor. Sólo al confirmar se descarta el trabajo local de esta pestaña. Cancelar conserva lo que estabas editando.';el('confirm-comparison').hidden=true;root.querySelector<HTMLElement>('.ps-note-label')!.hidden=true;const dialog=root.querySelector<HTMLDialogElement>('dialog')!;dialog.returnValue='';dialog.showModal();return;}
  if(target.dataset.do==='retry'&&pending&&!busy)void run(structuredClone(pending));
 },{signal});
 chrome();
 root.querySelector<HTMLDialogElement>('dialog')!.addEventListener('cancel',()=>{root.querySelector<HTMLDialogElement>('dialog')!.returnValue='';},{signal});
