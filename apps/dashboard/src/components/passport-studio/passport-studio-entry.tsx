...
export function PassportStudioEntry({initial,enrollment,endpoint}:{initial:StudioSnapshot|null;enrollment:StudioEnrollment|null;endpoint:string}){
 ...
 async function start(){
  ...
  try{const response=await fetch(`${endpoint}/start`,{method:'POST',headers:{'content-type':'application/json','idempotency-key':command.operationId},body:JSON.stringify(command),signal:controller.signal,cache:'no-store'});
 ...
 if(snapshot)return <PassportStudio key={`${snapshot.scope.tenantId}:${snapshot.scope.batchId}:${snapshot.draft.id}`} initial={snapshot} endpoint={endpoint}/>;
