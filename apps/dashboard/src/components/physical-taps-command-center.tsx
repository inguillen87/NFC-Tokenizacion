    ...
    >
      ...
      <div className="space-y-4 px-5 py-4">
        ...
        <div className="flex items-start gap-2 rounded-2xl border border-sky-300/15 bg-sky-400/[0.055] p-3 text-xs leading-5 text-slate-300">
          ...
          <span><b className="text-slate-100">{locationLabel(row)}</b><br />Zona aproximada por {row.location.source || "fuente de red"}; no es GPS ni prueba de presencia exacta.</span>
        </div>
...
export function PhysicalTapsCommandCenter({
  ...
  const summaryCards = [
    { label: "TAP de campo", value: number(payload.summary.total), detail: `${number(payload.summary.distinctUnits)} unidades · ${payload.scope.range || "ventana confirmada"}`, icon: Radio, tone: "text-cyan-200" },
    ...
    { label: "Zona disponible", value: `${located}/${rows.length}`, detail: "Aproximación IP · no GPS", icon: MapPin, tone: "text-sky-200" },
  ];
      ...
      <header className="flex flex-wrap items-start justify-between gap-4">
        <div className="max-w-3xl">
          <div className="flex flex-wrap items-center gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-full border border-cyan-300/25 bg-cyan-400/10 px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.14em] text-cyan-100"><ScanLine className="h-3.5 w-3.5" /> {tenantDisplayName} · campo</span>
          ...
          <h2 className="mt-3 text-2xl font-black tracking-tight text-white sm:text-3xl">TAP de campo recientes, listos para operar</h2>
          <p className="mt-2 text-sm leading-6 text-slate-300">Cada tarjeta representa una unidad diferente. Cerrado y abierto son observaciones independientes: no reconstruyen un antes/después ni una ruta del mismo producto.</p>
        </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[minmax(0,1.55fr)_minmax(20rem,.75fr)]">
        <div className="overflow-hidden rounded-3xl border border-white/10 bg-slate-950/48">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/8 px-4 py-3">
            <div>
              ...
              <p className="mt-1 text-xs text-slate-400">Marcadores agrupados a 3 decimales. Sin rutas, GPS ni precisión domiciliaria.</p>
            </div>
            <span className="rounded-full border border-sky-300/20 bg-sky-400/10 px-2.5 py-1 text-[10px] font-bold text-sky-100">IP aproximada</span>
          </div>
          {points.length ? (
            <PremiumVectorMap
              points={points}
              ...
              evidenceSteps={[
                ...
                { id: "privacy", label: "Privacidad", value: "Zona aproximada", detail: "IP; sin GPS ni domicilio", tone: "loyalty" },
        ...
        <aside className="space-y-3 rounded-3xl border border-violet-300/14 bg-violet-400/[0.045] p-4">
          ...
          <div className="grid gap-2 pt-1">
            <Link href={`/events?bid=${encodeURIComponent(payload.scope.bid)}&source=real`} className="inline-flex min-h-11 items-center justify-between rounded-xl border border-cyan-300/25 bg-cyan-400/10 px-3 py-2 text-xs font-black text-cyan-100 hover:bg-cyan-400/15"><span className="inline-flex items-center gap-2"><Radio className="h-3.5 w-3.5" /> Abrir evidencia técnica</span><ArrowRight className="h-3.5 w-3.5" /></Link>
