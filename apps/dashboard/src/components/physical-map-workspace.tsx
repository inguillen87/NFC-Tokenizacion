...
export function PhysicalMapWorkspace({ result, tenantSlug = "", dedicated = false, country = "" }: { result: PhysicalTapsResult; tenantSlug?: string; dedicated?: boolean; country?: string }) {
  ...
  const locationCounts=useMemo(()=>summarizeMapLocations(model.events),[model.events]);
  const mapEvents=useMemo(()=>filterMapLocations(model.events,sourceFilter),[model.events,sourceFilter]);
  const mapZones=useMemo(()=>{
    const zones=new Map<string,{key:string;city:string;country:string;taps:number;valid:number;risk:number}>();
    for(const event of mapEvents){const key=JSON.stringify([event.city,event.country]);const zone=zones.get(key)||{key,city:event.city||"Sin ciudad",country:event.country||"—",taps:0,valid:0,risk:0};zone.taps++;if(event.authenticationVerified)zone.valid++;if(isRealtimeRisk(event))zone.risk++;zones.set(key,zone);}
    return [...zones.values()].sort((a,b)=>b.taps-a.taps);
  },[mapEvents]);
  ...
  async function fullscreen() {
    ...
    catch { setNotice("Tu navegador no permite pantalla completa. El mapa sigue disponible en esta pantalla."); }
  }
  return <section id="mapa-profesional" className={`${styles.workspace} ${styles.compact} ${room.mapLayout}`} data-testid="physical-map-workspace">
      ...
      <div ref={panel} className={`${styles.mapPanel} ${expanded ? styles.expanded : ""}`}>
        <div className={styles.mapControls}>
          ...
          <div className={styles.links} role="group" aria-label="Controles geográficos">
            ...
            <button type="button" className={styles.button} aria-label="Restablecer mapa" onClick={() => {setZoom(1);setView("heat");setQuery("");setSeal("all");setSourceFilter("all");}}><RotateCcw size={16}/></button>
        ...
        <div className={styles.canvas}><MapCanvas events={mapEvents} hotspots={mapZones} mapView={view} baseMap={base} zoom={zoom} mode={tenantSlug ? "tenant" : "global"} dataState={model.confirmed ? "real" : "unavailable"} dataStateDetail="La base geográfica está disponible; las lecturas requieren sesión, permisos y una respuesta válida de la fuente." /></div>
      </div>
      <aside className={styles.side}>    <MapSourceControls value={sourceFilter} onChange={setSourceFilter} counts={{...locationCounts,none:model.withoutCoordinates+locationCounts.none}} />
    <div className={room.strip} aria-label="Resumen de la muestra geográfica"><span><b>{model.confirmed ? model.rows.length : "—"}</b>lecturas</span><span><b>{model.confirmed ? mapEvents.length : "—"}</b>coordenadas en esta capa</span><span><b>{model.confirmed ? model.withoutCoordinates : "—"}</b>sin coordenadas</span></div>
<h3><MapPin size={16} style={{display:"inline",marginRight:8}}/> Zonas de lectura</h3><p className={styles.note}>Zonas de la capa seleccionada. Red/IP no confirma la ciudad del teléfono.</p>{mapZones.length ? mapZones.map(point => <div className={styles.row} key={point.key}><b>{point.city}, {point.country}</b><span>{point.taps} lecturas · {point.valid} mensajes validados</span></div>) : <p className={styles.note}>{model.confirmed ? "No hay coordenadas utilizables para este filtro. Las lecturas sin ubicación siguen en la tabla." : "Fuente sin confirmar. No se sustituyen datos faltantes por una demo."}</p>}</aside>
    </div>
