  ...
  return (
    <Card className="p-5 text-sm text-slate-200">
      ...
      <div className="mt-3 flex gap-2 text-xs">
        <button suppressHydrationWarning type="button" className={`rounded-lg border px-2 py-1 ${mode === "demo" ? "border-cyan-300/35 bg-cyan-500/10 text-cyan-100" : "border-white/10 text-slate-300"}`} onClick={() => {
          setMode("demo");
          setBid(DEMO_SUPPLIER_BATCH_ID);
          setUidsText(DEMO_SUPPLIER_UIDS.join("\n"));
        }}>Demo mode</button>
        <button suppressHydrationWarning type="button" className={`rounded-lg border px-2 py-1 ${mode === "real" ? "border-emerald-300/35 bg-emerald-500/10 text-emerald-100" : "border-white/10 text-slate-300"}`} onClick={() => setMode("real")}>Producción real</button>
      </div>
      <div className="mt-3 grid gap-2 md:grid-cols-3">
        <input suppressHydrationWarning className="rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-xs text-white" placeholder="tenant slug" value={tenantSlug} onChange={(event) => setTenantSlug(event.target.value)} />
        <input suppressHydrationWarning className="rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-xs text-white" placeholder="tenant name" value={tenantName} onChange={(event) => setTenantName(event.target.value)} />
        <input suppressHydrationWarning className="rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-xs text-white" placeholder="BID" value={bid} onChange={(event) => setBid(event.target.value)} />
      </div>
      <textarea suppressHydrationWarning className="mt-3 min-h-24 w-full rounded-lg border border-white/10 bg-slate-950 px-3 py-2 text-xs text-white" value={uidsText} onChange={(event) => setUidsText(event.target.value)} placeholder="1 UID por línea o separadas por coma" />
