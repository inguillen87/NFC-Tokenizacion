  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <button suppressHydrationWarning className="rounded-xl border border-white/10 bg-slate-900 p-3 text-left text-sm text-white" onClick={async () => setOutput(JSON.stringify(await post('/scan', { bid: 'DEMO-2026-02', uidHex: '04B7723410E2AD', action: 'retail_scan', city: 'São Paulo', countryCode: 'BR' }), null, 2))}>Simulate replay attack</button>
        <button suppressHydrationWarning className="rounded-xl border border-white/10 bg-slate-900 p-3 text-left text-sm text-white" onClick={async () => setOutput(JSON.stringify(await post('/scan', { bid: 'DEMO-2026-02', uidHex: '04B7723410E2AE', action: 'uncork', city: 'Mendoza', countryCode: 'AR' }), null, 2))}>Simulate tamper alert</button>
