...
export function verifiedSessionPermissions(input:{role:string;tenantId?:string|null;tenantSlug?:string|null;isDemo?:boolean;permissions?:unknown}):string[]{
 ...
 if(isSupplierOperator(input.role)) return supplierOperatorPermissions(permissions);
...
import { isSupplierOperator, supplierOperatorPermissions } from "./supplier-operator-access";
