...
export function validateStudioReply(raw:unknown,command:StudioCommand,actorId:string):StudioReply{
 ...
 const target={save:'draft',submit:'in_review',request_changes:'changes_requested',approve:'approved',publish:'published'}[command.action];
 if(snapshot.draft.state!==target)throw new StudioRequestError('studio_receipt_state_invalid',true);
 if(command.action==='publish'&&!snapshot.published)throw new StudioRequestError('studio_publication_unconfirmed',true);
