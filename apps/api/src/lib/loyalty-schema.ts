...
async function migrateLoyaltySchema() {
  ...
  await sql/*sql*/`CREATE INDEX IF NOT EXISTS idx_loyalty_quizzes_tenant_status ON loyalty_quizzes(tenant_id, status, starts_at)`;
  await sql/*sql*/`CREATE INDEX IF NOT EXISTS idx_loyalty_quiz_attempts_tenant_created ON loyalty_quiz_attempts(tenant_id, created_at DESC)`;
  await sql/*sql*/`CREATE INDEX IF NOT EXISTS idx_loyalty_quiz_attempts_member_created ON loyalty_quiz_attempts(member_id, created_at DESC)`;
}
