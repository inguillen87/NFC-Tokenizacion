...
/**
 * Public SUN timelines never expose the exact coordinate persisted for an
 * event. Two decimals are intentionally coarser than the consented capture
 * used internally for operational diagnostics.
 */
...
function durableLocationEvidence(metadata: unknown) {
  ...
  return [objectRecord(sunContext?.geo), objectRecord(meta.geo_evidence)].filter(
...
function evidenceAccuracy(value: unknown) {
  ...
  return finiteNumber(evidence?.accuracy ?? evidence?.accuracy_m);
}
...
export function sanitizePublicLocationProjection(input: {
  ...
  if (source !== "browser_gps_approximate_consent") return { lat: null, lng: null };
  if (!["approximate", "browser_rounded"].includes(precision)) return { lat: null, lng: null };

  const consentEvidence = evidence.find((item) => (
    normalizedLocationToken(item.source) === "browser_gps_approximate_consent"
    && item.consent === true
