import {identityLookup,lookupIntakeIdentities} from './epcis-identity-picker';
...
export async function handleEpcisIntake(req:Request,bid:string,action:'read'|'preview'|'commit'|'identities',dependencies:{sessionResolver?:AdminSessionResolver;limit?:typeof enforceCriticalRateLimit}={}){
 const headers={'cache-control':'private, no-store','x-content-type-options':'nosniff'};
 try{
  if(req.method!==(action==='read'||action==='identities'?'GET':'POST'))throw new IntakeError('intake_method_not_allowed',405);
  ...
  const url=new URL(req.url);if(action!=='identities'&&([...url.searchParams.keys()].some(k=>k!=='tenant')||url.searchParams.getAll('tenant').length>1))throw new IntakeError('intake_query_invalid');
  ...
  const identityQuery=action==='identities'?identityLookup(url.searchParams,scope):null;
  ...
  if(action==='identities')return json({...await lookupIntakeIdentities(scope,identityQuery!),capabilities},200,headers);
