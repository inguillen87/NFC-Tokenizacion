...
export async function POST(req: Request) {
  ...
  const type = normalizeAlertType(String(body.type || ""));
  const severity = normalizeAlertSeverity(String(body.severity || "high")) || "high";
  if (!type) return json({ ok: false, error: "type_required" }, 400);
  if (!severity) return json({ ok: false, error: "invalid_severity" }, 400);
  const threshold = Number(body.threshold || 1);
  const windowMinutes = Number(body.window_minutes || 60);
  if (!Number.isFinite(threshold) || threshold <= 0) return json({ ok: false, error: "invalid_threshold" }, 400);
  if (!Number.isFinite(windowMinutes) || windowMinutes <= 0) return json({ ok: false, error: "invalid_window_minutes" }, 400);
  ...
  const rows = await sql/*sql*/`
    ...
    VALUES (${tenantId}, ${type}, ${severity}, ${threshold}, ${windowMinutes}, ${body.enabled !== false}, ${JSON.stringify(body.config || {})}::jsonb)
