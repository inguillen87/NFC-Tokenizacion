...
export async function PATCH(req: Request, context: { params: Promise<{ bid: string }> }) {
  ...
  const enrolled=await sql`SELECT batch_id FROM public.passport_editorial_workspaces WHERE batch_id=${batches[0].id}::uuid LIMIT 1`;
  if(enrolled[0])return json({ok:false,reason:"editorial_managed_use_studio",message:"Este lote utiliza revisión editorial. Continuá desde Passport Studio."},409);
  ...
  const updated = await sql/*sql*/`
    ...
    WHERE id = ${batches[0].id} AND sdm_config IS NOT DISTINCT FROM ${JSON.stringify(batches[0].sdm_config)}::jsonb
