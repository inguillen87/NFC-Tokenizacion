...
function renderSunHtml(contract: ReturnType<typeof buildPublicContract>, shareToken: string | null, locale: SunLocale) {
  ...
  <section class="card"><div class="section-head"><h3>${copy.identityPanel}</h3><span class="section-tag">Wine profile</span></div><p><b>${contract.product.name || 'Unprofiled product'}</b></p><p>${contract.product.winery || '-'} · ${contract.product.region || '-'}</p><div class="detail-grid"><div class="detail-item"><span class="k">Varietal</span><span class="v">${contract.product.varietal || '-'}</span></div><div class="detail-item"><span class="k">Vintage</span><span class="v">${contract.product.vintage || '-'}</span></div><div class="detail-item"><span class="k">Harvest</span><span class="v">${contract.product.harvestYear || '-'}</span></div><div class="detail-item"><span class="k">Barrel</span><span class="v">${contract.product.barrelMonths || '-'} months</span></div><div class="detail-item"><span class="k">Alcohol</span><span class="v">${contract.product.alcohol || '-'}</span></div><div class="detail-item"><span class="k">Serving</span><span class="v">${contract.product.serving || '-'}</span></div></div><p style="margin-top:10px">Bottle format: <b>${contract.product.bottle || '-'}</b></p></section>
  <section class="card"><div class="section-head"><h3>${copy.provenancePanel}</h3><span class="section-tag">Traceability</span></div><p>Origin: <b>${contract.provenance.origin || contract.iot.wineryLocation || '-'}</b></p><p>${copy.firstVerified}: <b>${contract.provenance.firstVerified.at || 'N/A'} · ${contract.provenance.firstVerified.city || '-'}, ${contract.provenance.firstVerified.country || '-'}</b></p><p>${copy.lastVerified}: <b>${contract.provenance.lastVerifiedLocation.at || 'N/A'} · ${contract.provenance.lastVerifiedLocation.city || '-'}, ${contract.provenance.lastVerifiedLocation.country || '-'}</b></p></section>
  <section class="card"><div class="section-head"><h3>${copy.iotPanel}</h3><span class="section-tag">Sensor intelligence</span></div><p>Winery: <b>${contract.iot.wineryLocation || 'N/A'}</b></p><p>Altitude: <b>${contract.iot.altitude || '-'}</b> · Oak: <b>${contract.iot.oakType || '-'}</b></p><p>Cellar temp: <b>${contract.iot.sensorSnapshot.cellarTemperature || '-'}</b> · Humidity: <b>${contract.iot.sensorSnapshot.humidity || '-'}</b></p><p>Light: <b>${contract.iot.sensorSnapshot.lightExposure || '-'}</b> · Transit: <b>${contract.iot.sensorSnapshot.transitShock || '-'}</b></p></section>
  <section class="card"><div class="section-head"><h3>${copy.tapPanel}</h3><span class="section-tag">Geo context</span></div><p>OS: <b>${contract.tapContext.os}</b> · Browser: <b>${contract.tapContext.browser}</b> · Device: <b>${contract.tapContext.deviceType}</b></p><p>Tap location: <b>${contract.tapContext.city || '-'}, ${contract.tapContext.country || '-'}</b>${contract.tapContext.lat != null && contract.tapContext.lng != null ? ` · (${contract.tapContext.lat}, ${contract.tapContext.lng})` : ''}</p><p style="font-size:11px;color:#94a3b8">User-Agent: ${contract.tapContext.userAgent || '-'}</p>
  <div class="map-grid"><div class="map-box"><p>Tap local map · zoom to winery route</p><iframe title="sun-tap-map" src="${mapEmbed}" loading="lazy" style="display:block;width:100%;height:180px;border:0"></iframe></div><div class="map-box"><p>Global context · tap visibility</p><iframe title="sun-global-map" src="${globalMapEmbed}" loading="lazy" style="display:block;width:100%;height:180px;border:0"></iframe></div></div>
  ...
  <section class="card"><div class="section-head"><h3>${copy.actionsPanel}</h3><span class="section-tag">Consumer journey</span></div><p class="subtitle" style="margin-bottom:10px">Consumer, warranty and traceability workflows.</p><div class="actions-grid" style="margin-bottom:8px"><a href="${contract.cta.marketplaceUrl}" data-gated-link="marketplace" class="link-btn" style="color:#a5f3fc;background:rgba(6,182,212,.12)">🛍 Marketplace ${contract.cta.clubName}</a><a href="${contract.cta.rewardsUrl}" data-gated-link="rewards" class="link-btn" style="color:#ddd6fe;background:rgba(139,92,246,.12)">🎁 Promos & rewards</a><a href="${contract.cta.registerUrl}" data-gated-link="register" class="link-btn" style="color:#d1fae5;background:rgba(16,185,129,.12)">🧾 Registrarme</a><a href="${contract.cta.portalUrl}" data-gated-link="portal" class="link-btn" style="color:#dbeafe;background:rgba(59,130,246,.12)">👤 Asociar al portal</a></div><div class="actions-grid"><button type="button" data-cta="claim-ownership" ${isReplay ? "disabled" : ""}>✓ ${copy.ctaClaim}</button><button type="button" data-cta="register-warranty" ${isReplay ? "disabled" : ""}>🛡 ${copy.ctaWarranty}</button><button type="button" data-cta="provenance">📍 ${copy.ctaProvenance}</button><button type="button" data-cta="tokenize-request" ${isReplay ? "disabled" : ""}>⛓ ${copy.ctaTokenize}</button></div><button id="nfc-scan" type="button" style="margin-top:8px;display:none">📲 Escanear con NFC</button><p id="cta-status" style="margin:10px 0 0;font-size:12px;color:#cbd5e1">${isReplay ? copy.statusReplay : copy.statusReady}</p><p style="margin:6px 0 0;font-size:11px;color:#94a3b8">Si el tap está verificado, al tocar estos botones te vamos a pedir registro/login y asociar tu usuario al tenant automáticamente.</p>${shareToken ? "" : '<p style="margin:8px 0 0;font-size:11px;color:#fbbf24">Demo mode: unsigned share fallback enabled for DEMO-* batches.</p>'}</section>
