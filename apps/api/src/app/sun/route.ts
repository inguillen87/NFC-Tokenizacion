...
function renderSunHtml(contract: ReturnType<typeof buildPublicContract>, shareToken: string | null, locale: SunLocale) {
  ...
  const riskStateLabel = isReplay
    ? `${labels.risk}: replay suspect`
    : productState === "VALID_MANUAL_OPENED" || contract.status.code === "MANUAL_OPENED"
      ? `${labels.risk}: manual opened`
      : productState === "VALID_OPENED" || contract.status.code === "OPENED"
      ? `${labels.risk}: tamper/opened`
      : productState === "VALID_OPENED_PREVIOUSLY" || contract.status.code === "OPENED_PREVIOUSLY"
        ? `${labels.risk}: opened previously`
      : `${labels.risk}: controlled`;
  ...
  <div class="map-grid"><div class="map-box"><p>${labels.mapLocalTitle}</p><iframe title="sun-tap-map" src="${mapEmbed}" loading="lazy" style="display:block;width:100%;height:180px;border:0"></iframe></div><div class="map-box"><p>${labels.mapGlobalTitle}</p><iframe title="sun-global-map" src="${globalMapEmbed}" loading="lazy" style="display:block;width:100%;height:180px;border:0"></iframe></div></div>
  <p style="margin:8px 0 0;font-size:11px;color:#94a3b8">${labels.routeSummary}: ${contract.iot.wineryLocation || 'Origin'} → ${contract.tapContext.city || '-'}, ${contract.tapContext.country || '-'} · ${labels.mapLegend}.</p></section>
