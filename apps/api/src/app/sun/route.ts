...
function renderSunHtml(contract: ReturnType<typeof buildPublicContract>, shareToken: string | null, locale: SunLocale) {
  ...
  <div class="world-map-wrap"><svg class="world-map-svg" viewBox="0 0 1000 460" role="img" aria-label="${labels.mapGlobalTitle}"><defs><linearGradient id="ocean" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stop-color="#0b244a"/><stop offset="100%" stop-color="#0a1d38"/></linearGradient><marker id="arrow" markerWidth="10" markerHeight="10" refX="8" refY="5" orient="auto"><path d="M0,0 L10,5 L0,10 z" fill="#f97316"/></marker></defs><rect width="1000" height="460" fill="url(#ocean)"/><g fill="rgba(125,211,252,.18)" stroke="rgba(125,211,252,.15)" stroke-width="1"><path d="M90 130 L230 80 L340 115 L355 170 L290 210 L160 200 Z"/><path d="M290 230 L365 255 L390 350 L330 420 L270 350 Z"/><path d="M430 90 L560 80 L650 110 L760 120 L850 170 L800 210 L690 195 L610 170 L520 165 L450 130 Z"/><path d="M640 235 L760 240 L845 285 L815 350 L700 360 L625 305 Z"/><path d="M835 350 L905 370 L875 420 L810 405 Z"/></g><line x1="${wineryPoint.x.toFixed(2)}" y1="${wineryPoint.y.toFixed(2)}" x2="${tapPoint.x.toFixed(2)}" y2="${tapPoint.y.toFixed(2)}" stroke="#f97316" stroke-width="3" stroke-dasharray="10 8" marker-end="url(#arrow)"/><circle cx="${wineryPoint.x.toFixed(2)}" cy="${wineryPoint.y.toFixed(2)}" r="8" fill="#22d3ee"/><circle cx="${tapPoint.x.toFixed(2)}" cy="${tapPoint.y.toFixed(2)}" r="8" fill="#f97316"/><text x="${Math.min(920, wineryPoint.x + 12).toFixed(2)}" y="${Math.max(20, wineryPoint.y - 10).toFixed(2)}" fill="#a5f3fc" font-size="12">${labels.origin}</text><text x="${Math.min(920, tapPoint.x + 12).toFixed(2)}" y="${Math.max(20, tapPoint.y - 10).toFixed(2)}" fill="#fdba74" font-size="12">${labels.tapLocation}</text></svg><div class="world-map-legend"><div class="legend-item"><span class="legend-dot legend-origin"></span><b>${labels.origin}</b><br/>${contract.iot.wineryLocation || "-"}</div><div class="legend-item"><span class="legend-dot legend-tap"></span><b>${labels.tapLocation}</b><br/>${contract.tapContext.city || '-'}, ${contract.tapContext.country || '-'}</div></div></div>
...
(() => {
  ...
  async function ensureAuthAndTenant(action) {
    ...
    if (!me?.ok) {
      ...
      const entered = window.prompt(ui.askCode, String(start.code || '')) || String(start.code || '');
