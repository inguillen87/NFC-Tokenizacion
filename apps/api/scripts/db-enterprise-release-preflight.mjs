...
export async function runEnterpriseReleasePreflight(options = {}) {
  ...
  try {
    const result = await client.query(
       `SELECT
          ...
          to_regprocedure('public.nexid_enterprise_rbac_risk_truth_v1_capability()') IS NOT NULL
            ...
            AND NOT EXISTS (
              ...
              FROM (VALUES
                ...
                ('operations_manager', true, true, '["supplier_order.create","manifest.import","packaging_lab.manage","qa.approve","batch.activate","batch.lifecycle","alerts.ack","events.read_sensitive","reports.export","rewards:validate"]'::jsonb),
                ('packaging_operator', true, true, '["packaging_lab.manage","manifest.import","reports.export"]'::jsonb),
                ('marketing_manager', true, true, '["batch.product.configure","consumer_experiences.read_pii","consumer_experiences.moderate","consumers.read_pii","leads.manage","reports.export","crm:read","campaigns:read","campaigns:write","rewards:read","marketplace:read"]'::jsonb),
         ...
         COALESCE(
           ...
           false
         ) AS has_identified_unverified_canonical_verdict,
         (
           SELECT COUNT(*) = 3
           FROM pg_trigger trigger_row
           WHERE NOT trigger_row.tgisinternal
         ...
         ) AS has_public_carrier_unit_consistency_guards,
    ...
    const requiredSchema = [
      ...
      ['identified_unverified canonical verdict', state.has_identified_unverified_canonical_verdict],
      ['public carrier unit consistency guards', state.has_public_carrier_unit_consistency_guards],
