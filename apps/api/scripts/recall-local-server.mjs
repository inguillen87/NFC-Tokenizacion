...
const port=Number(process.env.QA_HTTP_PORT||4291);
if(!Number.isSafeInteger(port)||port<1024||port>65535)throw new Error('Invalid local test port');
const f=await createRecallFixture(Number(process.env.QA_PG_PORT||15591));const calls=[];let loseNext=false;
...
});server.listen(port,'127.0.0.1',()=>console.log('S6_RECALL_ACTUAL_HANDLERS_LOCAL_'+port));
