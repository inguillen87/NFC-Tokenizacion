...
test("public QR scans fail closed on unknown tenant or batch and never create trusted context", () => {
  ...
  assert.match(qrSource, /publicReason = status === 503 \? "qr_context_unavailable" : "qr_context_not_found"/);
  assert.match(qrSource, /return failQrContext\(422/);
  assert.match(qrSource, /return failQrContext\(error\.status, error\.code\)/);
  ...
  assert.doesNotMatch(qrSource, /verdict:\s*"not_registered"/);
