...
test("packaging approval gate executes before ciphertext selection and every decrypt", async () => {
  ...
  const keySelectIndex = source.indexOf("JOIN batch_keys bk");
  const decryptIndex = source.indexOf("const kMetaHex = decryptBatchKeyHex(");
  ...
  assert.ok(decryptIndex > keySelectIndex, "key decrypt was not downstream of ciphertext query");
...
test("non-transactional order creation remains explicitly legacy until dedicated draft", async () => {
  const source = await readFile(createRoutePath, "utf8");
  assert.match(source, /caller-supplied packaging_spec is/);
