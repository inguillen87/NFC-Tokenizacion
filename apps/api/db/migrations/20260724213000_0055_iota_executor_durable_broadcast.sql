...
ALTER TABLE iota_executor_publications
  ...
  ADD CONSTRAINT iota_executor_publications_raw_transaction_check
    CHECK (raw_transaction IS NULL OR raw_transaction ~ '^0x([0-9a-f]{2})+$'),
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_iota_executor_publications_signer_nonce
  ON iota_executor_publications (chain_id, lower(signer_address), nonce)
  WHERE chain_id IS NOT NULL
    AND signer_address IS NOT NULL
    AND nonce IS NOT NULL
    AND status IN ('signed', 'broadcast', 'submitted');
