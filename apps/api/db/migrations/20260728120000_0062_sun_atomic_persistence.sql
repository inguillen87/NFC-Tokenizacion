-- Make the persistent SUN replay decision, tag counter transition, and event
-- append one PostgreSQL transaction. The HTTP API invokes this function with a
-- single statement; any exception rolls back every canonical state change.

DO $sun_uid_casefold_preflight$
BEGIN
  IF EXISTS (
    SELECT 1
    FROM tags
    GROUP BY batch_id, UPPER(uid_hex)
    HAVING COUNT(*) > 1
  ) THEN
    RAISE EXCEPTION USING
      ERRCODE = '23505',
      MESSAGE = 'sun_atomic_tag_uid_casefold_duplicates',
      HINT = 'Resolve case-insensitive duplicate UIDs explicitly before applying migration 0062; this migration never deletes or merges tag records.';
  END IF;
END
$sun_uid_casefold_preflight$;

CREATE UNIQUE INDEX IF NOT EXISTS uq_tags_batch_uid_upper
  ON tags (batch_id, UPPER(uid_hex));

CREATE INDEX IF NOT EXISTS idx_events_sun_payload_pair
  ON events (batch_id, picc_data_hash, cmac_hash, created_at, id)
  WHERE picc_data_hash IS NOT NULL AND cmac_hash IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_events_sun_raw_url_hash
  ON events (batch_id, raw_url_hash, created_at, id)
  WHERE raw_url_hash IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_events_sun_uid_counter_upper
  ON events (batch_id, UPPER(uid_hex), sdm_read_ctr, created_at, id)
  WHERE uid_hex IS NOT NULL AND sdm_read_ctr IS NOT NULL;
...
BEGIN
  ...
  v_pre_registry_result := NULLIF(UPPER(BTRIM(p_input->>'pre_registry_result')), '');
  v_auth_status := CASE
...
COMMENT ON FUNCTION public.nexid_persist_sun_scan_v1(jsonb) IS
  'Atomically serializes SUN replay detection, tag counters, and canonical event append. Software transaction boundary; not an HSM.';
