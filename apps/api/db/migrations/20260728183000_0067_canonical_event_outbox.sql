-- Canonical lifecycle/demo event writer with transactional tenant webhook outbox.
--
-- The physical SUN transaction remains owned by the prior atomic writer. This
-- migration only projects already-authorized application actions into the
-- canonical events stream. Tenant and batch identity are always resolved from
-- a persisted source event or batch; callers cannot supply a tenant id.
...
CREATE TABLE IF NOT EXISTS canonical_event_operations (
  ...
  CONSTRAINT canonical_event_operation_key_length_check
    CHECK (char_length(operation_key) BETWEEN 1 AND 255),
  CONSTRAINT canonical_event_request_fingerprint_check
    CHECK (request_fingerprint ~ '^[0-9a-f]{64}$'),
  CONSTRAINT canonical_event_name_check
    CHECK (event_name IN (
      'demo.tap.simulated',
  ...
  CONSTRAINT canonical_event_mode_check
    CHECK (event_mode IN ('live', 'demo', 'simulated')),
  CONSTRAINT canonical_event_partition_identity_check
    CHECK (event_id > 0 AND event_created_at IS NOT NULL),
  CONSTRAINT canonical_event_source_partition_identity_check
    CHECK ((source_event_id IS NULL) = (source_event_created_at IS NULL)),
  CONSTRAINT canonical_event_source_event_fkey
    FOREIGN KEY (source_event_id, source_event_created_at)
    REFERENCES events (id, created_at) ON DELETE RESTRICT,
  UNIQUE (tenant_id, operation_key)
);

CREATE INDEX IF NOT EXISTS idx_canonical_event_operations_event
  ON canonical_event_operations (event_id, event_created_at);

CREATE INDEX IF NOT EXISTS idx_canonical_event_operations_tenant_created
  ON canonical_event_operations (tenant_id, created_at DESC);

CREATE OR REPLACE FUNCTION nexid_reject_canonical_event_operation_mutation_v1()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  RAISE EXCEPTION 'canonical_event_operations_append_only';
END;
$$;

DROP TRIGGER IF EXISTS trg_canonical_event_operations_append_only ON canonical_event_operations;
CREATE TRIGGER trg_canonical_event_operations_append_only
BEFORE UPDATE OR DELETE ON canonical_event_operations
FOR EACH ROW EXECUTE FUNCTION nexid_reject_canonical_event_operation_mutation_v1();
...
BEGIN
  ...
  END IF;
  IF v_verdict NOT IN ('valid', 'invalid', 'replay_suspect', 'blocked_replay', 'revoked', 'broken', 'tampered', 'unknown_batch', 'not_registered', 'not_active') THEN
