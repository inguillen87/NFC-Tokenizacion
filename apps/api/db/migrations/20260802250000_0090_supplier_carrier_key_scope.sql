-- Rolling carrier-key scope upgrade.
--
-- Re-applies the atomic supplier-order writer so databases that already ran
-- migration 0079 gain the exact same profile-aware behavior as clean installs.
-- Static/identifier carriers have no SUN batch keys; the physical NTAG 424
-- DNA and TagTamper cryptographic path remains unchanged.
...
-- Enforce the invariant for every future supplier write, including code paths
-- outside the atomic order endpoint. NOT VALID avoids silently deleting or
-- rewriting historical rows; an operator must explicitly reconcile any
-- pre-0090 key material for static carriers before validating the constraint.
ALTER TABLE batches
  DROP CONSTRAINT IF EXISTS batches_supplier_carrier_key_scope_v1;
ALTER TABLE batches
  ADD CONSTRAINT batches_supplier_carrier_key_scope_v1 CHECK (
    supplier_order_id IS NULL
    OR (
      lower(COALESCE(carrier_profile_code, '')) IN ('ntag424_dna', 'ntag424_dna_tt')
      AND meta_key_ct IS NOT NULL
      AND file_key_ct IS NOT NULL
    )
    OR (
      lower(COALESCE(carrier_profile_code, '')) NOT IN ('ntag424_dna', 'ntag424_dna_tt')
      AND meta_key_ct IS NULL
      AND file_key_ct IS NULL
    )
  ) NOT VALID;
...
DECLARE
  v_carrier_profile_code text;
BEGIN
  IF NEW.supplier_order_id IS NULL THEN
    RETURN NEW;
  END IF;

  SELECT lower(supplier_order.carrier_profile_code)
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION USING ERRCODE = '23503', MESSAGE = 'supplier_batch_key_order_scope_invalid';
  END IF;
  IF v_carrier_profile_code NOT IN ('ntag424_dna', 'ntag424_dna_tt') THEN
    RAISE EXCEPTION USING ERRCODE = '23514', MESSAGE = 'supplier_batch_key_carrier_scope_forbidden';
  END IF;
  RETURN NEW;
END;
$carrier_key_scope$;
...
CREATE TRIGGER trg_batch_keys_supplier_carrier_scope_v1
  BEFORE INSERT OR UPDATE OF tenant_id, supplier_order_id ON batch_keys
...
CREATE TRIGGER trg_batch_key_material_supplier_carrier_scope_v1
  BEFORE INSERT OR UPDATE OF tenant_id, supplier_order_id ON batch_key_material
  FOR EACH ROW EXECUTE FUNCTION public.nexid_enforce_supplier_batch_key_carrier_scope_v1();
