...
type Example = { vertical: Vertical; point: MapKey; title: string; status: FeedEvent["status"] };
...
const i18n: Record<AppLocale, Copy> = {
  "es-AR": { kpiTags: "Tags activos", kpiValidations: "Validaciones hoy", kpiTamper: "Alertas tamper", kpiRegions: "Regiones activas", legendValid: "valid", legendTamper: "tamper", legendDuplicate: "duplicate", legendOpened: "opened", geoTitle: "Geo intelligence live feed", iconLabel: { wine: "🍷 Wine", events: "🎟️ Events", cosmetics: "🧴 Cosmetics", agro: "🌾 Agro", pharma: "💊 Pharma" }, examples },
  "pt-BR": { kpiTags: "Tags ativos", kpiValidations: "Validações hoje", kpiTamper: "Alertas tamper", kpiRegions: "Regiões ativas", legendValid: "valid", legendTamper: "tamper", legendDuplicate: "duplicate", legendOpened: "opened", geoTitle: "Geo intelligence live feed", iconLabel: { wine: "🍷 Wine", events: "🎟️ Events", cosmetics: "🧴 Cosmetics", agro: "🌾 Agro", pharma: "💊 Pharma" }, examples },
  en: { kpiTags: "Active tags", kpiValidations: "Validations today", kpiTamper: "Tamper alerts", kpiRegions: "Active regions", legendValid: "valid", legendTamper: "tamper", legendDuplicate: "duplicate", legendOpened: "opened", geoTitle: "Geo intelligence live feed", iconLabel: { wine: "🍷 Wine", events: "🎟️ Events", cosmetics: "🧴 Cosmetics", agro: "🌾 Agro", pharma: "💊 Pharma" }, examples },
};
...
export function RadarSection({ radar, locale }: { radar: RadarCopy; locale: AppLocale }) {
  ...
  useEffect(() => {
    const seed = txt.examples.slice(0, 7).map((item, i) => ({
      id: Date.now() + i,
      point: safePoint(item.point),
      vertical: item.vertical,
      title: item.title,
      status: item.status,
      at: new Date(Date.now() - i * 1200).toLocaleTimeString("en-GB"),
    }));
    setEvents(seed);

    const timer = setInterval(() => {
      setEvents((prev) => {
        const ex = txt.examples[Math.floor(Math.random() * txt.examples.length)] || txt.examples[0];
        if (!ex) return prev;
        const at = new Date().toLocaleTimeString("en-GB");
        return [{ id: Date.now(), point: safePoint(ex.point), vertical: ex.vertical, title: ex.title, status: ex.status, at }, ...prev.slice(0, 9)];
      });
      setScans((v) => v + 1);
    }, 1500);
  ...
  useEffect(() => {
    const onScenario = (event: Event) => {
      const custom = event as CustomEvent<{ mapKey?: string; vertical?: Vertical; label?: string; result?: string }>;
  ...
  const pulses = useMemo(() => events.filter((ev) => ev.point).slice(0, 7), [events]);
  ...
  return (
    <section className="container-shell py-16">
      ...
      <div className="mt-8 grid gap-5 xl:grid-cols-[1.35fr_0.65fr] xl:items-start">
        <Card className="relative overflow-hidden p-5 h-fit">
          ...
          <div className="radar-map-shell relative z-10 mt-4 h-[370px] overflow-hidden rounded-2xl border border-white/10 bg-slate-950/85">
            ...
            <svg viewBox="0 0 1000 500" className="absolute inset-0 h-full w-full opacity-95" aria-hidden>
              <g fill="rgba(56,189,248,0.18)" stroke="rgba(56,189,248,0.5)" strokeWidth="1.2">
              ...
              {routeArcs.map(([from, to], i) => (
                <path key={`${from}-${to}`} d={`M ${points[from].x * 10} ${points[from].y * 5} Q ${(points[from].x * 10 + points[to].x * 10) / 2} ${(points[from].y * 5 + points[to].y * 5) / 2 - 80} ${points[to].x * 10} ${points[to].y * 5}`} fill="none" stroke="rgba(34,211,238,0.48)" strokeWidth="2" strokeDasharray="6 7" style={{ animation: `arcFlow 3.2s linear ${i * 0.15}s infinite` }} />
              ))}
            </svg>
            ...
            {pulses.map((ev) => (
              <div key={ev.id} className="absolute -translate-x-1/2 -translate-y-1/2" style={{ left: `${ev.point.x}%`, top: `${ev.point.y}%` }}>
                ...
                <div className="mt-1 rounded-md border border-cyan-300/40 bg-slate-900/95 px-2 py-1 text-[10px] text-slate-100">{ev.point.city}</div>
              </div>
            ))}
            <div className="absolute bottom-3 left-3 text-[11px] text-slate-300">{radar.mapCaption}</div>
          </div>

          <div className="relative z-10 mt-3 grid gap-2 md:grid-cols-2 xl:grid-cols-4 text-[11px]">
