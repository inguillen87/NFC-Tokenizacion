...
export default async function TapsTimelinePage({searchParams}:{searchParams?:Promise<Record<string,string|string[]|undefined>>}){
 ...
 try{const h=await headers(),r=await fetch(productUrls.api+'/consumer/taps/history?'+historyParams(filters),{cache:'no-store',signal:AbortSignal.timeout(12000),headers:{cookie:h.get('cookie')||'','user-agent':h.get('user-agent')||'nexid-web-history'}});if(r.ok)initial=parseHistory(await boundedHistoryJson(r),{query:filters,page:1});}catch{}
