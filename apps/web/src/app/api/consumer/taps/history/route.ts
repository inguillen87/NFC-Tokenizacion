...
export async function GET(req:Request){
 ...
 try{
  const r=await fetch(productUrls.api+'/consumer/taps/history?'+q,{cache:'no-store',signal:AbortSignal.timeout(12000),headers:{cookie:stripConsumerTapCapabilityCookies(req.headers.get('cookie')),'user-agent':req.headers.get('user-agent')||'nexid-consumer-history'}});
