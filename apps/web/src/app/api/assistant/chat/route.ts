...
function localAnswer(locale: string, question: string, leadSaved: boolean) {
  ...
  if (locale === "en") {
    const answers: Record<string, string> = {
      ...
      reseller: "For resellers we package hardware + encoding + SaaS + onboarding. The value story is recurring revenue, CRM leads, analytics exports, tenant dashboards and a white-label rollout model. Start with Bodega Balmec, show source-labelled demo events, then quote volume and tag profile.",
  ...
  const answers: Record<string, string> = {
    ...
    reseller: "Para revendedores, el paquete es hardware + encoding + SaaS + onboarding. La historia de valor es margen inicial, MRR, CRM de leads, exportaciones, dashboard por tenant y rollout white-label. Arranca con Bodega Balmec, mostrá eventos demo con fuente identificada y después cotizá volumen + perfil de tag.",
