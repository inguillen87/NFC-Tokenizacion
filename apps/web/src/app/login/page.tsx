  ...
  return (
    <main className="container-shell grid min-h-screen place-items-center">
      <Card className="w-full max-w-md p-8">
        <h1 className="text-2xl font-bold text-white">{content.auth.loginTitle}</h1>
        <p className="mt-2 text-sm text-slate-400">{content.auth.loginBody}</p>
        <div className="mt-6 grid gap-3">
          <input className="rounded-xl border border-white/10 bg-slate-950 px-3 py-2" placeholder={content.auth.email} />
          <input type="password" className="rounded-xl border border-white/10 bg-slate-950 px-3 py-2" placeholder={content.auth.password} />
