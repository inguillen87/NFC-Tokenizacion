...
test("the page accepts authentic VALID_CLOSED-style states without requiring generic VALID status", async () => {
  ...
  assert.match(page, /const isManualOpenedState = statusCode === "MANUAL_OPENED"[\s\S]*?productState === "VALID_MANUAL_OPENED"/);
  assert.match(page, /const isVerifiedOpenedState = !isManualOpenedState[\s\S]*?"VALID_OPENED_PREVIOUSLY"\]\.includes\(statusCode\)/);
  assert.match(page, /const sealOpened = !isManualOpenedState/);
