// Loaded only by the S7 disposable QA processes, never by the application.
const allowed = new Set(['localhost', '127.0.0.1', '::1', '[::1]']);
function inspect(value) {
  const host = typeof value === 'string' || value instanceof URL
    ? new URL(value).hostname
    : String(value?.hostname || value?.host || 'localhost').replace(/:\d+$/, '');
  if (!allowed.has(host)) throw Error('S7_QA_NON_LOOPBACK_NETWORK_BLOCKED');
}
