# FAQ enterprise de confianza, Polygon e IOTA

Este FAQ alinea el discurso comercial y tecnico de nexID para clientes enterprise. Debe usarse cuando se explica autenticidad NFC, ownership en Polygon, proof layer IOTA, privacidad y auditoria.

## Respuesta corta

nexID valida productos fisicos con NFC criptografico y registra eventos DPP en su backend. Polygon se usa para propiedad digital, NFT, certificados y claims. IOTA puede usarse como capa opcional para hashes de auditoria, DPP y logistica. Los datos sensibles no se publican on-chain y no todos los taps se escriben en blockchain.

## Preguntas frecuentes

### Cada tap va a blockchain?

No. Cada tap relevante se registra en nexID para auditoria, antifraude y analytics. Solo algunos eventos se tokenizan o anclan segun politica del tenant.
...
Polygon es la capa de ownership. Se usa para emitir o registrar certificados, NFTs, claims y transferencias controladas.

La pregunta que responde Polygon es: "quien tiene o reclamo el certificado digital de este producto?".

### Que rol cumple IOTA?

IOTA es una capa opcional de prueba y auditoria. Se usa para anclar hashes o checkpoints de eventos DPP, manifest, logistica o calidad.
...
- Hashes o digests de eventos seleccionados.
...
Para IOTA, se solicita el evento autorizado, se canonicaliza, se calcula el digest y se compara contra el digest anclado.
...
- "IOTA puede activarse como proof layer opcional para evidencia DPP y logistica."
...
- "Alianza formal con una red" sin respaldo publico.
