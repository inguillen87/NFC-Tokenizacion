...
- Web `3b501b6bf3b2b08dc55f1fe12742c57f27f57574`, release `2026.09.08-web.3`, deployment `dpl_8N2Ej1SzbX58hoCFCwhJk8WcadZK`, URL inmutable `https://nexid-6tg7hyeyg-marcelos-projects-c26aa499.vercel.app`. Build Next.js y TypeScript aprobados, inspección READY y promoción posterior. `/release.json` coincide en `nexid.lat` y `nexid.com.ar`.
- QA autenticado real de producción: historial con cinco registros asociados, apertura de #566 (Cabernet Franc Reserva 2022, Mendoza, 25 de junio) y #404 desde billetera (Gran Reserva Malbec, San Martin, 30 de mayo). Son registros anteriores, no taps nuevos de septiembre. Se verificaron modo claro/oscuro, productos, menú Más y destino privado desde billetera. Sin `Risk Level: NaN` ni enlaces a certificados públicos sin firma.
- Revisión independiente de código sin bloqueadores: sesión, propiedad y empresa, límites de campos, IDs bigint y caché privada. No sustituye la prueba de aislamiento productivo con dos cuentas distintas.
- Consulta de errores Vercel para ambos deployments desde las 20:06 UTC hasta el cierre del QA: sin registros de error devueltos por CLI autenticada. El conector MCP devolvió 403 antes de consultar; no se interpretó ese rechazo como ausencia de errores. El navegador tampoco devolvió errores en las últimas ocho entradas solicitadas. Es una ventana de QA, no certificación de disponibilidad continua.
- Detectado para el siguiente incremento: la billetera rotulaba los puntos de red como canjeables en bodega y convertía ausencia de cuenta en cero, aunque mostraba 540 puntos de Balmec en otra sección. Los ámbitos son distintos; no deben sumarse ni usarse uno como reemplazo del otro.
...
- QA: 537/537 pruebas web y TypeScript aprobados. Fixture exclusivamente local con marca 120/180, red deshabilitada, ausencia de fuente y recuperación mediante Reintentar. Capturas claro/oscuro a 390 px CSS verificados y sin desborde horizontal; son emulaciones de navegador, no un teléfono físico.
- La capacidad general de viewport no redimensionó Chrome; se usó emulación de dispositivo acotada a la pestaña local y se retiró. Las capturas móviles no se usan como prueba de interacción táctil. Un click bajo la primera emulación alcanzó Salir; sólo ocurrió en el fixture local (POST deshabilitado 503). Se verificó el cambio de tema en tamaño normal y después la presentación móvil.
- Pendiente de otro sprint: revisar el resumen de `/me/rewards`, que aún calcula un agregado de puntos de varias marcas, y la disponibilidad de métricas de productos. No se presenta esta corrección acotada de billetera como cierre de todos los módulos de beneficios.

## Fase 6: beneficios privados y comprobantes coherentes

- Web `2026.09.08-web.5`: puntos por marca sin agregados entre empresas; búsqueda, filtro por marca y vistas Todos / Mis vouchers / Publicados. Se distinguen vouchers emitidos, vencidos, cancelados y catálogo. Código copiable sólo para un voucher propio vigente; un parámetro de URL no prueba propiedad ni habilita el canje. El costo histórico usa `points_spent`, no el precio actual.
- API `2026.09.08-api-portal.2`: lectura autenticada privada, máximo 100 filas entre reclamos propios y catálogo de marcas asociadas. Reclamos unidos por consumidor, empresa y recompensa; no descubre catálogo de otras empresas por visibilidad de red. Respuestas 401/503 privadas, sin convertir fallos en listas vacías. La consulta de beneficios no ejecuta inicialización del portal ni DML; el helper de autenticación conserva su inicialización previa de autenticación.
- Corrección financiera acotada: los movimientos de puntos insertan su saldo final desde filas bloqueadas, sin intentar actualizar una fila recién insertada dentro de la misma sentencia WITH. La creación del comprobante pertenece a la misma operación; una colisión inesperada revierte puntos y stock. La cancelación emite su movimiento y estado conjuntamente.
- PostgreSQL real aislado: rama schema-only `br-misty-star-ainwxwdp`, endpoint `ep-snowy-recipe-aip9m3hz`, base exclusiva `codex_qa_rewards_20260908`, caducidad de rama 2026-09-09 23:00 UTC. No se copiaron filas productivas. Pasaron 26 escenarios financieros (concurrencia, idempotencia, último cupo, rollback) y 11 de lectura (propiedad, empresa, estados, vigencia y límite). Ambos arneses eliminaron únicamente sus schemas temporales y cerraron todas sus conexiones. Sin cambios a variables de entorno persistidas.
- QA local: 554/554 pruebas web, 43/43 suite rewards de API y 262/262 de seguridad; TypeScript de ambas aplicaciones y revisión de diff aprobados. Las nuevas pruebas rewards forman parte del build API. Las 37 pruebas PostgreSQL son adicionales y no se ejecutan contra producción.
- Navegador local aislado: búsqueda sin coincidencias, limpiar filtros, filtros por marca y tipo, detalles, copiar código, fuente indisponible, recuperación mediante Reintentar y estado vacío. Claro/oscuro y emulación móvil con ancho CSS 390/documento 390; sin desborde horizontal ni errores de consola en la ventana consultada. No representa una prueba en un teléfono físico.
- Límite intencional: la nueva UI no habilita claim/cancel/self-redeem ni presenta `can_claim:false` como bloqueo del POST existente. Los handlers previos todavía deben incorporar elegibilidad completa, validación por personal y revisar la inicialización que repone stock de demobodega; por eso no se certifica el canje integral ni se prueba consumiendo puntos reales. El vencimiento se proyecta para la lectura, no cambia el estado persistido. No hay paginación más allá de las 100 filas ni polling agregado.
- Publicado: API commit `29c4f623`, deployment `dpl_76TRRjoH8xggAaq37aAJF8Cbs5eA`, URL inmutable `https://nexid-bjnuubfy0-marcelos-projects-c26aa499.vercel.app`; WEB commit `869ecdf0`, deployment `dpl_6G2bPMnoATxY9ySf5ZQufKNWbNp6`, URL inmutable `https://nexid-bqxi2xw6q-marcelos-projects-c26aa499.vercel.app`. Builds Next.js 16.2.12 completos (API 59 s, WEB 51 s), inspeccionados READY y promovidos API primero y WEB después a las 22:57 UTC. Los deployments CLI no expusieron `meta.githubCommitSha`; se registran los commits locales revisados y subidos, sin afirmar metadata Git inexistente. El cambio ajeno de `next-env.d.ts` no fue incluido en el commit.
- `/release.json`: API `.2` y WEB `.5` confirmados en `api.nexid.lat`, `nexid.lat` y `nexid.com.ar`. La URL inmutable API mantiene el rechazo por origen; no se deshabilitó. Sonda canónica sin sesión: `/consumer/rewards` devuelve 401 `unauthorized` y `no-store, private`.
- QA autenticado productivo: 540 puntos de Bodega Balmec y seis registros, uno de ellos voucher histórico `2x1 copa bienvenida Bodega Balmec`, vencido el 25 de junio, sin código de canje activo. Mis vouchers filtra ese registro, su detalle conserva fecha/estado y Limpiar filtros recupera los seis. Se verificaron claro/oscuro y navegación a Billetera, que conserva saldo y acumulados 540/540. No se reclamaron premios, cancelaron vouchers ni consumieron puntos reales.
- Logs de ambos deployments desde 22:57 UTC hasta el cierre del QA: CLI autenticada no devolvió registros de error. El navegador sí registra advertencias y un error de extensiones de wallets (`chrome-extension://`), separados de la aplicación. `GET /v1/drains` devolvió lista vacía; no hay forwarding externo confirmado y esta revisión acotada no equivale a monitoreo continuo.
